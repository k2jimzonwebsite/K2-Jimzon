...
Release checkout check: the isolated `codex/listing-release` checkout contains only this Admin listing slice and its records; other local account, Storefront and database work was excluded. `npm run verify:release` exited 0 after the final test-fixture correction, with 954/954 base tests, 23/23 intake UI tests, every intermediate browser suite, separate Storefront/Admin builds, source boundaries and secret scans passing. The Admin application measured 217.34 kB against its 300.00 kB minified budget. An earlier gate run found a missing Admin-only Gemini variable allowance and stale Admin route counts; both exact failure areas passed 7/7 after correction. A later run exposed the manual fixture's missing post-Draft consignment read; its focused case passed after the fixture response was added. This receipt proves the prepared release checkout locally; GitHub/Vercel state requires separate confirmation.
