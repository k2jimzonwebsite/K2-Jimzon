...
**25 September human testing source release:** `f625381ccd25dba84a1e2279bc6b9a72f0eac742` fast-forwarded GitHub `main` after the complete local release gate exited 0. GitHub deployments `6642848306` (Storefront) and `6642833359` (Admin) report success for that exact SHA. Canonical Storefront and Admin build markers both returned HTTP 200 with their own target. GitHub CI run `36027662677` was still running when the deployment receipts were recorded. The prepared account/notification SQL and signed guest changes were committed as source only; no Supabase migration or feature flag changed. Human purchase, recipient, payment, intake and recovery checks remain in MAP-017 through MAP-025. Recovery is a scoped revert and restoration of both prior successful Vercel artifacts before any provider cutover. Detailed evidence: `docs/evidence/20260925-human-testing-release/README.md`.

**24 September signed-in browser recheck:** Hostinger, Vercel, Supabase and GitHub were inspected through the K2 Chrome account. Hostinger's four web DNS records still match section 4 below. Vercel displayed separate Ready production projects and Valid Configuration for `www.k2jimzon.com`, the apex 308 redirect, and `admin.k2jimzon.com`. Both project overviews displayed GitHub source commit `2d6f7cfd9a380bce5871064de7044c28eefbe346`, the documentation-only commit after the scoped code release. GitHub showed `main` at that commit, code CI run `35955278280` successful, and no `main` protection rule. An attempted narrow default-branch rule stopped at GitHub's email identity verification prompt; returning to Rulesets confirmed none was saved. Supabase displayed Healthy and latest migration `20260921033348 admin_globe_direct_rpc`; its prepared guest/account chain remains unapplied. Auth Site URL and the `www`, apex and Admin redirect patterns are configured, while new-user sign-up is off. The Free plan backup page states that scheduled project backups are unavailable and point-in-time recovery requires a paid add-on. The live sitemap and robots returned 200, with four approved public URLs, and representative route headers preserved private/product/Admin noindex. Search Console access was rejected by automatic browser review pending explicit Google-service authorization; Google ownership/submission/indexing, GA4 and Cloud remain unverified. No DNS, project, database, Auth or repository setting was changed. Evidence and next actions: `docs/evidence/20260924-provider-readiness-browser/README.md`, MAP-017/019/020/022/024/028.
...
This confirms current production content on both hosts. GitHub deployment
receipts `6630128794` (Storefront) and `6630134932` (Admin) both report
`success` for this exact SHA. Their Vercel artifact URLs are
`https://k2-jimzon-eptvuc3mv-k2-jimzon.vercel.app` and
`https://k2-jimzon-admin-fwv45qh1n-k2-jimzon.vercel.app`. GitHub CI run
`35955278280` passed `build-and-smoke` and `critical-sql-behavior` for the
same SHA. A narrow live check returned HTTP 200 with expected content types
for the Storefront home, Pasabuy page, sitemap, both QR images, and the Admin
entry. The configured Vercel connector exposed only an unrelated team, so no
Vercel project API was used.
...
113,000 handles; it was interrupted. The local aggregate release gate remains
**incomplete**, while remote CI passed. A focused source-contract run passed
59/59. Do not treat this
as proof of a real order, real funds, staff acceptance, search indexing, or a
real payment receipt. No database migration, provider switch, payment
