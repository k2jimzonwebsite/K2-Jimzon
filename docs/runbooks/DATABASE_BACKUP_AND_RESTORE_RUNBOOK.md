...
**Current MAP-017 stock correction recovery receipt, 25 September:** The owner explicitly authorized the encrypted envelope and its two redacted companion files in the existing unshared `K2 Production Backups` folder (ID `1mQuU8Jj6eWhDr-lpZV3YJDtaEwfAh8yo`, owner `k2jimzonwebsite@gmail.com`). Envelope Drive ID `1iPIOt0Z-sd8g4a3g4zhJj5VGgwKHOb3M`, manifest ID `1KuzmSauVKJrLGltQUAgZR39c48CVnbkS`, restore receipt ID `1xJFze343q47tGBrRyNZqfn8mYFsHxMe6`. Metadata readback showed the expected parent, owner, unshared state and sizes; independent raw envelope download matched local SHA-256 `DDE4432C4355A6462D52A8FBDC875A71EB7831298270EEB5CD9C21C5DBA99F9B`, and both companions parsed on independent readback. The exact stock migration and emergency rollback passed on an isolated restore before the live apply. Production migration `20260925111537` then removed only PostgreSQL `PUBLIC` EXECUTE on `get_public_product_stock()`; postflight retained named `anon`/`authenticated` grants and catalog availability. The emergency rollback is `supabase/migrations/20260925_map017_stock_public_grant_revocation_rollback.sql` (SHA-256 `61A929C103298F43DB96E819EF1C6E3E8DCBE7DEF61EAEC8161CBB26B6B5C470`); use it only if public stock access fails after verifying the named grants. Application-schema restore did not restore ten managed Vault entries, Storage objects or provider settings, and owner-held recovery access has not been independently exercised. The earlier approval-review block paragraphs below are dated history. Full evidence: `docs/evidence/20260925-map017-stock-apply/README.md`.
...
**25 September owner-authorized envelope upload:** Drive file `1iPIOt0Z-sd8g4a3g4zhJj5VGgwKHOb3M` now holds the encrypted envelope in the unshared owner-only K2 Production Backups folder. Metadata readback matched the parent, owner and 825,060 bytes; independent raw download matched SHA-256 `DDE4432C4355A6462D52A8FBDC875A71EB7831298270EEB5CD9C21C5DBA99F9B`. The separate 686-byte redacted manifest was rejected by automatic approval review as an upload outside the user's envelope-only authorization. Without its authenticated-data fields, the offsite envelope alone is not a complete restore package. Await explicit owner authorization for the manifest and restore receipt, then upload and verify both in the same folder before live apply. No production SQL changed.
