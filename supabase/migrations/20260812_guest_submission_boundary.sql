...
create table if not exists k2_private.guest_conversation_receipts (
...
declare
  ...
  v_message text;
begin
  if p_action not in ('order', 'pasabuy', 'coupon', 'guest_start', 'guest_read', 'guest_reply') then
...
begin
  ...
  insert into public.conversations(
    ...
    'Website','Open','website_message',v_key,1,now(),now()+interval '4 hours'
...
$$;
revoke all on function public.start_guest_conversation_v1(bigint,uuid,text,text,text,text)
  from public,anon,authenticated;
