-- Rollback for 20260916_automated_delivery_quotation.sql
-- Restores submit_guest_order_v1 to 20260912_guest_order_conversation_seed baseline.

begin;

-- Preflight: submit_guest_order_v1 must exist
do $preflight$
begin
  if to_regprocedure('public.submit_guest_order_v1(bigint,uuid,text,text,text,text)') is null then
    raise exception 'PREFLIGHT_FAILED: public.submit_guest_order_v1 must exist before applying rollback';
  end if;
end
$preflight$;
...
begin
  if to_regprocedure('public.submit_guest_order_v1(bigint,uuid,text,text,text,text)') is null then
    raise exception 'POSTFLIGHT_FAILED: public.submit_guest_order_v1 was not found after rollback';
  end if;
end
$postflight$;
