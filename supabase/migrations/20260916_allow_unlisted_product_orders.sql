-- ===========================================================================
-- MAP-023 Queue item 12: Honour documented meaning of 'Unlisted' status
--
-- 'Unlisted' products are hidden from browse but reachable and buyable by direct
-- link. This migration updates submit_order_request_v2 to accept 'Unlisted'
-- alongside 'Live' and 'Active', matching StoreContext.jsx and InventoryGrid.jsx.
--
-- Applied after: 20260902_purchase_time_reservation.sql
-- ===========================================================================
...
begin
  if to_regprocedure('public.submit_order_request_v2(text,text,text,text,text,text,jsonb,text,text)') is null then
    raise exception 'PREFLIGHT_FAILED: public.submit_order_request_v2 must exist before applying 20260916_allow_unlisted_product_orders.sql';
  end if;
end $$;
...
begin
  if to_regprocedure('public.submit_order_request_v2(text,text,text,text,text,text,jsonb,text,text)') is null then
    raise exception 'POSTFLIGHT_FAILED: public.submit_order_request_v2 was not found after migration';
  end if;
end $$;
