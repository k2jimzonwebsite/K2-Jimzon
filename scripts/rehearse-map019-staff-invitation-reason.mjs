...
const data=path.join(root,'.tools','map019-staff-invite-pg-data')
const log=path.join(root,'.tools','map019-staff-invite-pg.log')
const port='55434'
const database='k2_map019_staff_invite_rehearsal_local'
...
try {
  ...
  run('psql',[...target,'-f','supabase/tests/map019_staff_invitation_reason_bootstrap.sql'],'bootstrap',env)
  run('psql',[...target,'-f','supabase/migrations/20260824_admin_staff_invitation_reason_boundary.sql'],'migration',env)
  run('psql',[...target,'-f','supabase/tests/map019_staff_invitation_reason_behavior.sql'],'behavior assertions',env)
  run('psql',[...target,'-f','supabase/migrations/20260824_admin_staff_invitation_reason_boundary.sql'],'idempotent replay',env)
  console.log('MAP-019 staff invitation portable rehearsal passed: reason retention, replay, conflict, privileges, and migration replay verified.')
} finally {
