...
**Next slice:** the scoped `get_public_product_stock()` correction is prepared but unapplied: `supabase/migrations/20260925_map017_stock_public_grant_revocation.sql` (SHA-256 `479BCCE8AFE8EEA68B7C89F85370F5EE06FC28268ADC7DF9BC155D236E88A94D`) removes only the extra PostgreSQL `PUBLIC` execute grant while retaining explicit anonymous and authenticated access; the emergency-only rollback is SHA-256 `61A929C103298F43DB96E819EF1C6E3E8DCBE7DEF61EAEC8161CBB26B6B5C470`. The 25 September read-only live check confirmed `PUBLIC=true`, named `anon=true`, named `authenticated=true`, and the migration ledger still ending at `20260921033348`. The local runner was corrected to recognize its already-running localhost practice database by data directory after sandboxed `pg_ctl status` falsely reported no server; `npm run rehearse:stock-grant` then passed revoke, replay, read-only check and rollback. `tests/map017-stock-grant-contract.spec.js` passed 4/4. Owner action is only to approve or defer this exact live correction under OWNER-005; Codex must refresh backup/recovery and live preflight, apply in a controlled window, independently check grants and exact-host catalog, and record or recover an ambiguous result. Prepare the signed guest replacement before revoking the six transitional anonymous RPC grants. Follow up the Supabase support request for the six `supabase_admin` default-privilege findings. The restored current-schema rollback rehearsal covers archived ACLs and named grant denials; prove signed preview continuity and managed-role behavior before a live cutover. Classification and audit reports: `docs/evidence/20260924-account-migration-rehearsal/`.
