...
**Next executable slice:** after explicit owner approval of the existing Drive backup destination, upload and independently verify the new encrypted envelope, recheck the live stock ACL, apply only the scoped grant correction, then prove named grants, public RPC and exact-host catalog continuity. In parallel, prepare the signed guest replacement and prove preview continuity before proposing revocation of the six transitional anonymous RPC grants. Follow up the Supabase support request for the six `supabase_admin` default-privilege findings. The restored current-schema rollback rehearsal covers archived ACLs and named grant denials; prove managed-role behavior before a guest cutover. Classification and audit reports: `docs/evidence/20260924-account-migration-rehearsal/`.
...
**Block:** the guest cutover needs MAP-019/020. Supabase support has logged a request from the K2 account for the supported correction of provider-owned defaults, with project support access off; its answer is pending. A new 25 September encrypted application-database backup `current-pixplcjqivlfflickobf-2026-09-25T095512876Z-688de96720d7` was restored and verified in the dedicated local `k2_current_restore_20260925_stock` database; its redacted manifest and restore receipt are under ignored `.tools/map017-stock-preapply-20260925.k2backup*`. This local backup has not been uploaded or independently retrieved; refresh it if live state changes before apply. The 24 September database and Storage backups passed isolated restores and were uploaded to the owner-only Drive folder; the earlier database envelope was independently fetched and SHA-256 matched. An ordered MAP-019/020 rollback-only SQL chain passed on the earlier current schema both without and with archived ACLs; named direct-RPC denials and signed guest grants passed, then the baseline returned. The local grant-preserving restore needed non-login placeholders for missing managed roles and still excluded Vault, so provider role behavior is not fully reproduced. Storage-part retrieval, provider recovery, signed preview continuity and owner recovery-access checks remain open. Evidence: `docs/evidence/20260913-audit-remediation/`, `docs/evidence/20260924-account-migration-rehearsal/` and the database recovery runbook.
