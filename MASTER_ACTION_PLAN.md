...
**25 September 06:05 UTC update:** The owner reports Admin is signed in, but Codex computer use returns `User unavailable`; AAL2 and the staff screen remain unobserved. The fresh production migration ledger still ends at `20260921033348`, a read-only relation/function probe confirms MAP-018 objects remain absent, and the full read-only MAP-018 preflight executed without error. Continue from the MAP-017 permission gate before applying the intake migrations; reconnect the owner browser for real-host acceptance. Evidence: `docs/evidence/20260925-inventory-readiness/README.md`.

**25 September logic audit and correction:** The authoritative sequence is scan/identity check → package evidence → manual ChatGPT or optional paid API content → human review → one server-assigned Draft → separate stock → separate publication. The newer modal presents scan as Step 1 and both content choices after evidence; opening its resumable session first is technical setup, but the production table is absent. The older chooser instead labels an existing-SKU barcode prompt as “automatic” intake and new-product Smart Paste as “manual”; “Scan & Intake Now” saves no stock. Automatic API requires the inactive Admin BFF and OWNER-007 paid-AI decision. The owner reports no specific failed control or error; the earlier claim that an existing workflow had broken was an unsupported agent inference. Verify the existing workflow before diagnosing a regression. Correct the misleading chooser in the MAP-018 activation work without restoring direct product/lot writes. Evidence: `docs/evidence/20260925-inventory-readiness/README.md`.
