...
**Status:** Active. Phase one and the approved `20260909023000` follow-up are applied and independently verified. A fresh read-only production metadata export on 24 September produced 18 audit findings (13 critical, 5 high). Five authenticated functions had their existing Staff or Admin/AAL2 guards and grants classified in the repository contract; re-audit of the same export has 13 critical and 0 high. No live grant changed. The earlier ten-critical count is dated evidence.

**Next slice:** prepare a scoped correction for `get_public_product_stock()` that removes only its extra PostgreSQL `PUBLIC` execute grant, retaining explicit anonymous and authenticated access. A separate read-only repository check detects this broader grant without changing the hash-bound historical postflight; production still has it. Prepare the signed guest replacement before revoking the six transitional anonymous RPC grants. Follow up the Supabase support request for the six `supabase_admin` default-privilege findings. The restored current-schema rollback rehearsal now covers archived ACLs and named grant denials; prove signed preview continuity and managed-role behavior before a live cutover. Classification and audit reports: `docs/evidence/20260924-account-migration-rehearsal/`.
...
**Block:** the guest cutover needs MAP-019/020. Supabase support has logged a request from the K2 account for the supported correction of provider-owned defaults, with project support access off; its answer is pending. On 24 September, fresh encrypted database and Storage backups passed isolated local restores and were uploaded to the owner-only Drive folder. The database envelope was independently fetched and SHA-256 matched. An ordered MAP-019/020 rollback-only SQL chain passed on the current schema both without and with archived ACLs; named direct-RPC denials and signed guest grants passed, then the baseline returned. The local grant-preserving restore needed non-login placeholders for missing managed roles and still excluded Vault, so provider role behavior is not fully reproduced. Storage-part retrieval, provider recovery, signed preview continuity and owner recovery-access checks remain open. Evidence: `docs/evidence/20260913-audit-remediation/`, `docs/evidence/20260924-account-migration-rehearsal/` and the database recovery runbook.

### MAP-018: Verify products, intake and inventory

**IDEA-20260925-01 cleanup slice:** The owner wants one scan-first product-intake lifecycle and a reversible archive of obsolete code. Audit Inventory and Sheet entry points, guides and tests against the rulebook before moving anything. Keep a dated archive with original paths, hashes and restore instructions; archive only sources proven unused after the canonical routes are wired. Verify scan → duplicate/known SKU branch → evidence → manual or optional API content → review → Draft → separate stock/publication in focused browser tests. Do not merge or deploy a cleanup that would replace the currently usable production path before MAP-017/020 database and server gates pass. Documentation and release state must say prepared versus live explicitly.

**25 September candidate evidence and next slice:** Branch `codex/intake-cleanup` routes Inventory/Sheet launches to the prepared secure session when enabled and to the current scanner/ChatGPT/Smart Paste path when disabled. It connects Sheet's missing scan-to-paste handoff, opens lot review for exact matches, removes the unreachable direct browser product-insert branch, and archives the old chooser. Focused contracts passed 46/46, `npm run verify:development` passed, Admin build/budget/boundary/secret checks passed, and the guarded Admin browser suite passed 6/6. These are local/fixture results; no live staff intake, database migration, provider call, or deployment was verified. Next: run a real signed preview with the MAP-017/020 prerequisites, then physically scan one known and one new product through evidence, manual JSON review, Draft, separate lot, and publication checks; verify paid API only after OWNER-007. Keep the production flag-off manual path available until that acceptance and the full release gate pass. Recovery: restore the archived files and `originals/` snapshots in `archive/2026-09-25-intake-and-unused-ui/`, or revert the candidate commit.

**Draft PR #14 CI correction:** Both Vercel previews and critical SQL behavior passed on the first commit, but base CI failed one workflow-guide truth case because the guide still named the retired “Smart paste” toolbar button. The active guide now names “Scan box”; its exact contract passed 9/9 locally. The old guide is preserved in the dated archive. Recheck the new CI run before treating PR checks as green.
...
**IDEA-20260925-01 repository cleanup slice:** Inventory candidate dead code by import graph plus named runtime/CLI/route entry points. Move only independently verified inactive sources to a dated archive; keep migration, runbook, evidence and recovery artifacts in their authoritative locations. Record every moved original path and hash, run import and focused contract checks, then one `verify:development` after the final code edit. A broad unreferenced-file scan alone is not proof of dead code.

**25 September candidate evidence and next slice:** Of 229 `src` JS/JSX/CSS/JSON files, 219 were reachable from the three app entries plus the Admin worker; all 10 graph candidates received reference checks. Only unimported `adminKit.jsx` was archived as runtime-unused. The chooser was archived because the owner retired its pre-scan choice, not because it was dead. The remaining eight graph candidates were retained for server, Vite, script, contract, or gated-feature use; the two unmounted scanner components need a feature-status and workflow audit before any move. Archive paths, original hashes and restore instructions are in `archive/2026-09-25-intake-and-unused-ui/README.md`. Focused contracts 46/46, development gate and Admin artifact checks passed locally. Next: inspect unmounted scanner workflows and dated docs by MAP dependency order, then run the complete release gate and exact-host checks before any promotion. No production cleanup is claimed.
