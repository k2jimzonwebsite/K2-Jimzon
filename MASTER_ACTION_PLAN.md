...
**25 September live check and recovery point:** Owner-authenticated, read-only Supabase SQL confirmed the stock function still has `PUBLIC` execute plus explicit `anon`/`authenticated` grants. A new encrypted application database backup restored to an isolated local PostgreSQL database (51 public relations; 10 managed entries excluded); a new encrypted Storage backup restored 36 product-image files with matching fingerprints. Eight encrypted/redacted artifacts were uploaded to the owner-only Drive folder and checked for size, parent and owner-only permissions. Remote byte retrieval/hash, provider settings, Vault, and grant-preserving ACL replay remain unverified. No production SQL was applied. Evidence and IDs: `docs/evidence/20260925-inventory-readiness/README.md`.

**Next slice:** re-export the live ACL and rehearse this exact correction and recovery against the current grant-preserving restored schema, including managed-role limits; obtain a fresh verified backup and controlled change authorization before any production apply. A separate read-only repository check detects the broader grant; production still has it according to the 24 September export. Prepare the signed guest replacement before revoking the six transitional anonymous RPC grants. Follow up the Supabase support request for the six `supabase_admin` default-privilege findings. Prove signed preview continuity and managed-role behavior before a live cutover. Classification and prior rehearsal: `docs/evidence/20260924-account-migration-rehearsal/`.
