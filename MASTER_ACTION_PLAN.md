...
**25 September route discrepancy:** The owner expects both manual AI copy/paste and automatic API intake to start with a scan. The deployed source also exposes an older chooser: its “Scan & Intake Now” prepares an AI prompt and opens Smart Paste without recording stock, while the newer shared-session modal starts before the scan and cannot load because the production intake table is absent. Automatic API also requires the inactive Admin BFF and OWNER-007 paid-AI decision. Confirm the exact failing control with the owner; correct the misleading chooser and unify the scan-to-Draft handoff only with the canonical MAP-018/MAP-020 activation. Do not restore a direct product/lot write as a shortcut. Evidence: `docs/evidence/20260925-inventory-readiness/README.md`.
