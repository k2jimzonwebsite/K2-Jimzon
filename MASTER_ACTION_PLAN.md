...
**Owner decision, 25 September:** the owner deferred the scoped `get_public_product_stock()` correction. Keep `supabase/migrations/20260925_map017_stock_public_grant_revocation.sql` (SHA-256 `479BCCE8AFE8EEA68B7C89F85370F5EE06FC28268ADC7DF9BC155D236E88A94D`) prepared and unapplied; do not apply or re-request it unless the owner reopens this decision. The 25 September read-only live check confirmed `PUBLIC=true`, named `anon=true`, named `authenticated=true`, and the migration ledger still ending at `20260921033348`. The corrected local runner passed revoke, replay, read-only check and rollback; the focused contract suite passed 4/4. This is local evidence only and the stock finding remains live.

**Later 25 September owner reopening and apply preparation:** The owner replied “bro fix it,” reopening the earlier deferral for the reviewed stock payload. Fresh read-only production preflight confirmed function owner `postgres`, broad `PUBLIC` plus both named grants, 21 stock rows, and ledger ending at `20260921033348`. The public-key RPC returned HTTP 200 with 21 rows; `https://www.k2jimzon.com/catalog` returned HTTP 200. Fresh encrypted backup `current-pixplcjqivlfflickobf-2026-09-25T100842718Z-dde4432c4355` passed isolated restore (51 public relations; 10 managed Vault entries excluded), and the exact stock migration and emergency rollback passed on the restored application schema with stock row count preserved at 21. The restored local database was removed. **Recovery gate blocked:** automatic approval review rejected uploading this sensitive backup to the previously used unshared owner-only K2 Drive folder because this destination lacked explicit trusted user authorization. Do not route around that rejection or apply production SQL before the offsite backup is authorized, uploaded and verified. OWNER-005 records the single owner decision needed; the backup remains encrypted locally under ignored `.tools/map017-stock-liveapply-20260925.k2backup*`.

**Later 25 September backup progress:** The owner explicitly authorized the encrypted envelope upload to the existing K2 Production Backups folder. Drive file `1iPIOt0Z-sd8g4a3g4zhJj5VGgwKHOb3M` was uploaded there; metadata readback showed 825,060 bytes, unshared, owner `k2jimzonwebsite@gmail.com`, and the correct folder parent. Independent raw download SHA-256 matched local `DDE4432C4355A6462D52A8FBDC875A71EB7831298270EEB5CD9C21C5DBA99F9B`. Automatic approval review then rejected upload of the **separate redacted manifest** because the owner's wording covered only the encrypted envelope. The manifest is needed to authenticate/decrypt this backup after local loss, so production apply remains stopped. Ask only for explicit authorization to upload the redacted manifest and restore receipt to the same folder; no workaround. Live read-only ACL recheck still showed the broad grant, both named grants and 21 stock rows. No production SQL changed.
