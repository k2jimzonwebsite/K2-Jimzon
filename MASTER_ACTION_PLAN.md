...
**Status:** Active. Phase one, `20260909023000`, and the 25 September scoped stock-grant correction are applied and independently verified. The latest production export reports **12 critical, 0 high**: six transitional direct guest RPC grants and six provider-owned `supabase_admin` default-privilege groups. Earlier counts and the deferred/upload-blocked paragraphs below are dated history, superseded by the applied receipt.

**Historical owner decision, superseded:** The owner initially deferred the scoped stock grant correction. That decision was later reopened and fulfilled by the applied receipt below. The prepared migration and prior local checks are preserved in Git history and the MAP-017 evidence.

**Historical preparation, superseded:** Fresh live preflight, encrypted backup and isolated migration/rollback rehearsal passed. Automatic approval review initially blocked offsite upload until the owner specified the destination. No production SQL changed at this stage; the later applied receipt below is current.

**Historical backup progress, superseded:** The owner authorized the encrypted envelope upload; its independent Drive download hash matched. Automatic approval review then required separate approval for the redacted manifest and restore receipt. The owner later gave that approval, and all three files were verified before the apply recorded below.

**25 September scoped stock correction applied (current state):** The owner separately authorized both backup companion files. The encrypted envelope, redacted manifest and restore receipt were uploaded to the same unshared owner-only folder and independently read back (Drive IDs `1iPIOt0Z-sd8g4a3g4zhJj5VGgwKHOb3M`, `1KuzmSauVKJrLGltQUAgZR39c48CVnbkS`, `1xJFze343q47tGBrRyNZqfn8mYFsHxMe6`). After a fresh preflight, the exact scoped migration above applied successfully to production as ledger `20260925111537`. Independent SQL confirmed `PUBLIC` execute absent and named `anon` and `authenticated` execute retained, with 21 stock rows. Public RPC returned HTTP 200/21 rows and the exact-host catalog displayed 22 products with availability. Fresh production metadata audit reports **12 critical, 0 high**, down from 13 critical. This closes the stock-grant slice only; see `docs/evidence/20260925-map017-stock-apply/README.md`.

**25 September provider advisor triage needed:** A fresh Supabase security advisor read reports seven RLS-without-policy informational findings, seven anonymous and 39 authenticated executable `SECURITY DEFINER` warnings, and one leaked-password-protection warning. These are separate advisor notices, with some intentional public/role-gated paths and overlap with the schema audit. Classify each against actual grants, function guards and exposure before calling it a vulnerability or closing it. MAP-017 owns function/RLS classification; MAP-019/021 own the Auth password-protection setting. Evidence and counts: `docs/evidence/20260925-production-readiness/README.md`.

**Next executable slice:** Prepare the signed guest replacement and prove preview continuity before proposing revocation of the six transitional anonymous RPC grants. Follow up the Supabase support request for the six `supabase_admin` default-privilege findings. The restored current-schema rollback rehearsal covers archived ACLs and named grant denials; prove managed-role behavior before a guest cutover. Classification and audit reports: `docs/evidence/20260924-account-migration-rehearsal/` and `docs/evidence/20260925-map017-stock-apply/`.
...
**Block:** provider controls and owner recovery access must be exercised on the current artifacts. The scoped stock database envelope was independently retrieved and hash-checked, but Storage-part retrieval, managed Vault/provider configuration, owner-held decryption access and live service recovery are unproven. See `docs/runbooks/DATABASE_BACKUP_AND_RESTORE_RUNBOOK.md`.
...
**25 September delivery/rate/payment audit (IDEA-20260925-06):** The exact live checkout still shows region/weight fees (NCR standard ₱95, express ₱150, pickup free) and GCash/MariBank, with no completed order. Production has no Admin delivery-rate tables or signed delivery command, so staff cannot publish general rates today. The separate live per-order `set_order_delivery_details` RPC allows staff to replace a previously accepted fee without a ceiling; no authenticated save was tested. The anonymous direct order RPC trusts a browser-supplied amount of ₱0–₱100,000 as `customer_confirmed`, without checking approved rate/route. Deployed Admin source sends four payment RPC arguments, but production exposes only the legacy three-argument function and no independent-verifier enforcement; do not claim payment evidence can be recorded or verified through the current direct UI. Local focused delivery/payment contracts pass 70/70, which does not establish live behavior. Reconcile the regional-matrix versus exact-locality J&T implementations under the owner's clarified policy, then make pricing server-authoritative, preserve accepted fees, coordinate Admin BFF/schema activation and prove two-person real payment and fulfillment. Full evidence and recovery: `docs/evidence/20260925-delivery-payment-workflow-audit/README.md`.
...
**Status:** Active. GCash and MariBank QR choices are deployed, but no real transfer has been independently verified. The feature branch locally fixes the receipt method switch and stale payment wording. Browser-side regional delivery calculation is deployed; server validation, Admin rate publishing, booking and cost reconciliation remain unproven.

**Next real-world acceptance slice, after the engineering guards and payment boundary above:** the owner and staff scan both exact QRs on a second device and check their recipients against receiving accounts. With counted stock, submit one real order, confirm stock and total, make one small transfer, record evidence, and have a different authorized reviewer confirm the funds. Rehearse packing, dispatch, courier cost, exceptions, refunds and customer status. Keep Cash on Delivery off until its prepared availability boundary is applied and accepted.
...
**25 September release receipt, refreshed:** Source release `f625381` passed `npm run verify:release`, separate Vercel target checks, and GitHub CI run `36027662677` now `completed/success`. Both latest production-target deployments are `READY` on later documentation commit `57604cd` (`[skip ci]`); no second source release is inferred. Account and guest BFF switches remain off, and their migrations are unapplied. Current matrix: `docs/evidence/20260925-production-readiness/README.md`. Human acceptance remains pending.

**Next slice:** track each finding through its owning item above. Docs slice IDEA-20260925-01 (no source change): archive the 18 unreferenced August evidence papers to a dated folder with a restore note, keep the two pinned papers in place, and update the intake spec boundary wording to the single phone-scan logic. Code reroute stays a later slice behind MAP-018. Remaining: `vercel.json` headers and Content-Security-Policy-Report-Only observability; canonical `sitemap` and robots signals; channel vocabulary and oversell protection; guest and linked-account recovery on the exact host; complete Admin phone navigation and readability with staff; and collection of role, device, accessibility, performance and rollback evidence. Retest the exact release candidate before another owner-requested promotion.
...
**Block:** MAP-017 through MAP-025. Source release SHA `f625381ccd25dba84a1e2279bc6b9a72f0eac742` has separate Vercel receipts and successful CI; the latest production-target deployment is documentation SHA `57604cdcf17362bb9601d61c4140f717bc421e53`. Broader launch remains unaccepted. See the deployment runbook and `docs/evidence/20260925-human-testing-release/README.md`.
