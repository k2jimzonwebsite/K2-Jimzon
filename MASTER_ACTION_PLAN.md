...
**Status:** Active. The 24 September Storefront audit found account, notification, continuity, receipt, payment copy, robots and allergen issues. The isolated release candidate repairs receipt/copy/robots/allergen behavior and prepares account/profile/notification behavior. It also merges the older guided intake and optional dashboard Help pilots into current Admin navigation. Production activation and human proof remain open.

**Current release slice:** The isolated `codex/human-testing-release` candidate passed the final `npm run verify:release` gate, including all browser suites, both target builds and security/budget checks. Stage only reviewed source and documents, push the resulting commit to production-linked `main`, and verify both exact deployment receipts and host markers. Keep customer and guest BFF flags off, and do not apply the prepared account, moderation or guest cutover migrations as part of a Git push. Follow the MAP-017/019/020 provider sequence before real account testing. Generated screenshots from older test runs are excluded. Earlier interrupted gates and the fixture correction are recorded in `docs/evidence/20260925-human-testing-release/README.md`. Production and human acceptance remain pending.
...
**Block:** MAP-017 through MAP-025. Scoped release SHA `5494b2bf71f7ecf30422e2b8c5bfc246b895bc9f` has CI and separate Vercel receipts; broader launch remains unaccepted. See the deployment runbook and `docs/evidence/20260924-storefront-audit-fixes/README.md`.
