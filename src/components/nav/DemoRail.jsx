    ...
    <>
      ...
      <div className="sticky top-0 z-50 hidden border-b border-line bg-shell md:block">
        <div className="mx-auto flex h-10 max-w-6xl items-center gap-1 px-4">
          <span className="mr-3 text-[10px] font-semibold uppercase tracking-[0.24em] text-navy-faint">
            Prototype
          </span>
          {VIEWS.map((v) => (
            <button
          ...
          <span className="ml-auto flex items-center gap-2 text-[11px] text-navy-faint">
      ...
      <nav
        ...
        className="fixed inset-x-0 bottom-0 z-50 border-t border-line bg-white/95 pb-[env(safe-area-inset-bottom)] backdrop-blur md:hidden"
      >
          ...
          {VIEWS.map((v) => {
            ...
            return (
              <button
                ...
                onClick={() => go(v.id)}
                className={
                  'flex flex-col items-center gap-1 pb-2.5 pt-1.5 text-[9px] font-medium transition-colors ' +
