...
export default function ProductDetail() {
  const { productId, getProduct, addToCart, setCartOpen, isWholesale, lines } = useStore()
  ...
  const remaining = Math.max(0, (product.stock_available || product.stock) - inCart)
              ...
              <div className="relative min-h-[400px] lg:min-h-0">
                <AnimatePresence mode="wait">
                  {!showRecipe ? (
                    ...
                    >
                      <div className="flex flex-wrap items-center gap-2 mt-4 lg:mt-0 shrink-0">
                        <TrustBadge>100% authentic · {product.origin || 'Italy'}</TrustBadge>
                        <StockPill stock={product.stock_available || product.stock} />
                      </div>
                      
                      <div className="shrink-0">
                        <h1 className="mt-4 font-serif text-2xl font-semibold leading-tight tracking-tight text-navy sm:text-3xl md:text-4xl lg:text-5xl">
                          {product.name}
                        </h1>
                        <p className="mt-2 flex items-center gap-2 text-sm text-navy-soft">
                          <StarIcon className="text-gold" /> 4.9 · 812 sold across our channels · {product.size || 'Premium'}
                        </p>
                      </div>

                      <div className="mt-6 flex items-baseline gap-4 shrink-0">
                      ...
                      <div className="flex-1 mt-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pb-4">
                          <div className="relative rounded-xl bg-shell p-5 shadow-sm border border-line/40">
                            ...
                            <p className="mt-2 text-sm leading-relaxed text-navy-soft">{product.whyBuy || "Superior quality ingredients that elevate any dish to restaurant standards."}</p>
                          </div>
                          ...
                          <div className="relative rounded-xl bg-shell p-5 shadow-sm border border-line/40">
                            ...
                            <p className="mt-2 text-sm leading-relaxed text-navy-soft">{product.usage_instructions || product.whyRare || "Perfect for both professional kitchens and home cooking."}</p>
                          </div>
                        </div>
                      </div>
                    ...
                    >
                      <div className="mb-6 mt-4 lg:mt-0 shrink-0">
                        ...
                        <h2 className="font-serif text-xl sm:text-2xl font-bold tracking-tight text-navy lg:text-3xl">
                          {product.guide.title}
                        </h2>
                      </div>
