...
export default function PurchaseOrders() {
  ...
  return <div className="space-y-5">
    ...
    <div className="overflow-x-auto rounded-adm-sm border border-adm-line bg-adm-sunken"><table className="w-full min-w-[780px] text-left text-sm text-white/65"><thead className="bg-white/5 text-xs uppercase tracking-wider text-white/45"><tr><th className="px-5 py-3">PO number</th><th className="px-5 py-3">Supplier</th><th className="px-5 py-3">Expected</th><th className="px-5 py-3">Lines</th><th className="px-5 py-3">Amount</th><th className="px-5 py-3">Status</th><th className="px-5 py-3 text-right">Receiving</th></tr></thead><tbody className="divide-y divide-adm-line">{loading ? <tr><td colSpan="7" className="px-5 py-8 text-center text-white/45">Loading purchase orders…</td></tr> : orders.length === 0 ? <tr><td colSpan="7" className="px-5 py-8 text-center text-white/45">No purchase orders recorded.</td></tr> : orders.map(order => <tr key={order.id}><td className="px-5 py-4 font-mono text-white">{order.po_number}</td><td className="px-5 py-4">{order.suppliers?.name || 'Not recorded'}</td><td className="px-5 py-4">{order.expected_delivery || 'Not recorded'}</td><td className="px-5 py-4">{order.po_lines?.length || 0}</td><td className="px-5 py-4 font-mono text-white">{peso(Number(order.total_amount || 0))}</td><td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${order.status === 'Received' ? 'bg-forest/15 text-forest' : order.status === 'Cancelled' ? 'bg-crimson/15 text-crimson' : 'bg-blue/15 text-blue'}`}>{order.status}</span></td><td className="px-5 py-4 text-right">{order.status === 'Received' ? <span className="inline-flex items-center gap-1 text-xs text-forest"><CheckIcon size={14} /> Recorded received</span> : <span className="text-xs text-white/35">Use consignment receiving with lot and expiry</span>}</td></tr>)}</tbody></table></div>
  </div>
}
