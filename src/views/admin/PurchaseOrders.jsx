...
export default function PurchaseOrders({ secureMode }) {
  ...
  return <div className="space-y-5">
    <div className="flex flex-col justify-between gap-3 border-b border-adm-line pb-4 sm:flex-row sm:items-end"><div><h2 className="font-sans text-xl text-white">Purchase orders</h2><p className="mt-1 max-w-[65ch] text-[15px] leading-relaxed text-white/75">Saved supplier commitments and expected quantities: who you bought from, how many, at what cost. Movement lives under Consignments.</p></div><span className="text-[13px] leading-relaxed text-white/60">PO creation and batch-aware receiving are not enabled in this screen yet.</span></div>
    {error && <div role="alert" className="flex items-start gap-2 rounded-adm-sm border border-crimson/40 bg-crimson/10 p-3 text-sm text-crimson"><AlertIcon size={17} /><span>{error}</span></div>}
