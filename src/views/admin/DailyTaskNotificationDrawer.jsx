...
export default function DailyTaskNotificationDrawer({ isOpen, onClose, onNavigate }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div className="flex h-full w-full max-w-md flex-col border-l border-adm-line bg-adm-surface text-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="flex shrink-0 items-center justify-between border-b border-adm-line bg-adm-sunken px-5 py-4">
          <div>
        ...
        <div className="flex-1 space-y-3 overflow-y-auto p-4">
          {loading ? (
            ...
            batches.map((b) => (
              <div key={b.id} className="rounded-adm-sm border border-adm-line bg-adm-surface p-3.5">
                <div className="flex items-start justify-between gap-2">
                  <p className="text-sm font-semibold text-white">{b.product_name || b.sku}</p>
                  <span className={'shrink-0 rounded border px-2 py-0.5 text-xs font-bold ' + (TONE[b.status] || TONE.warning)}>
