...
const NAV_GROUPS = [
  ...
  { heading: 'Buying & shipments', items: ['kanban', 'consignment', 'pasabuy_manager', 'suppliers'] },
  { heading: 'Products & customer work', items: ['inventory', 'store_assets', 'omni_hub', 'reservations', 'inbox', 'wholesale', 'coupons'] },
  { heading: 'Channels & access', items: ['delivery', 'staff_permissions', 'integrations', 'globe'] },
]

function activeNavGroup(section) {
  if (section === 'overview') return 'group-0'
  const index = NAV_GROUPS.findIndex(group => group.items.includes(section))
  return index < 0 ? 'group-0' : `group-${index}`
}

function NavList({ section, onSelect, activeSkus, canManageStaff, widget, onWidget, collapsible = false }) {
  const [openGroup, setOpenGroup] = useState(() => activeNavGroup(section))

  useEffect(() => {
    if (collapsible) setOpenGroup(activeNavGroup(section))
  }, [collapsible, section])

  return (
    <div className={collapsible ? 'space-y-1' : 'space-y-5'}>
      {collapsible && <button type="button" aria-expanded={openGroup === 'widgets'} aria-controls="admin-mobile-widgets"
        onClick={() => setOpenGroup(openGroup === 'widgets' ? null : 'widgets')}
        className="flex min-h-11 w-full items-center justify-between rounded-adm-sm px-3 text-left text-sm font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue/70">
        Dashboard views {openGroup === 'widgets' ? <MinusIcon size={16} /> : <PlusIcon size={16} />}
      </button>}
      <nav id={collapsible ? 'admin-mobile-widgets' : undefined} hidden={collapsible && openGroup !== 'widgets'} aria-label="Dashboard widgets" className="space-y-0.5 border-b border-adm-line pb-3">
        {!collapsible && <p className="px-3 pb-2 text-xs font-medium text-white/65">Dashboard widgets</p>}
      ...
      {NAV_GROUPS.map((group, gi) => {
        ...
        return <div key={groupId}>
          {collapsible ? <button type="button" aria-expanded={open} aria-controls={`admin-mobile-${groupId}`}
            onClick={() => setOpenGroup(open ? null : groupId)}
            className="flex min-h-11 w-full items-center justify-between rounded-adm-sm px-3 text-left text-sm font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue/70">
            {group.heading || 'Home & guides'} {open ? <MinusIcon size={16} /> : <PlusIcon size={16} />}
          </button> : group.heading && (
            <p className="px-3 mb-1.5 text-xs font-semibold uppercase tracking-wider text-white/55">
              {group.heading}
            </p>
          )}
          <div id={collapsible ? `admin-mobile-${groupId}` : undefined} hidden={collapsible && !open} className="space-y-0.5">
  ...
  return (
    <div className="admin-ui flex min-h-screen bg-adm-bg pb-20 text-white/80 md:pb-0 font-sans selection:bg-blue/30 selection:text-white">
      ...
      <main className="min-w-0 flex-1 flex flex-col h-full overflow-hidden">
        {/* Mobile Header */}
        {/* The short section name leaves room for tools; the workspace heading
            below names the current task. */}
        <div className="flex min-h-[58px] w-full shrink-0 items-center justify-between gap-2 border-b border-adm-line bg-adm-sunken px-3 lg:hidden">
          <h1 ref={mobileHeadingRef} tabIndex={-1} className="min-w-0 truncate text-base font-semibold text-white focus:outline-none">{meta.label}</h1>
          <div className="flex items-center gap-0.5 shrink-0">
            ...
            <button ref={mobileMenuTriggerRef} aria-label="Open navigation menu" onClick={() => { setMobileToolsOpen(false); setIsMobileMenuOpen(true) }} className="flex min-h-[44px] min-w-[44px] items-center justify-center text-white/50 transition-colors hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue/70">
              <MenuIcon size={21} />
            </button>
          </div>
        </div>
        ...
        {isMobileMenuOpen && (
          <div className="fixed inset-0 z-50 flex lg:hidden">
            ...
            <AdminDialog onClose={() => setIsMobileMenuOpen(false)} returnFocusRef={mobileMenuTriggerRef} labelledBy="admin-mobile-nav-title">
            <div className="relative flex w-72 max-w-[88vw] flex-col overflow-y-auto border-r border-adm-line bg-adm-sunken px-2 pb-20 pt-4 shadow-adm-float">
              <div className="flex items-center justify-between px-3 mb-4">
                <div><p className="text-sm font-semibold text-white/65">K2 Jimzon BOS</p><h2 id="admin-mobile-nav-title" className="mt-0.5 text-base font-semibold text-white">Admin navigation</h2></div>
              ...
              <NavList collapsible widget={widget} onWidget={id => { setWidget(id); selectSection('overview') }} section={section} onSelect={selectSection} activeSkus={activeSkus} canManageStaff={canManageStaff} />
              <div className="mt-3 border-t border-adm-line pt-2">
                <button type="button" aria-expanded={mobileToolsOpen} aria-controls="admin-mobile-staff-tools"
                  onClick={() => setMobileToolsOpen(open => !open)}
                  className="flex min-h-11 w-full items-center justify-between rounded-adm-sm px-3 text-left text-sm font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue/70">
                  Staff tools {mobileToolsOpen ? <MinusIcon size={16} /> : <PlusIcon size={16} />}
                </button>
                <div id="admin-mobile-staff-tools" className={mobileToolsOpen ? 'space-y-0.5' : 'hidden'}>
                {section === 'inventory' && <>
                  <button type="button" onClick={() => { setIsMobileMenuOpen(false); setShowCsvImport(true) }}
                    className="flex min-h-11 w-full items-center gap-2.5 rounded-adm-sm px-3 text-left text-sm text-white/70 hover:bg-white/[0.04] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue/70">
                    <UploadIcon size={16} className="shrink-0" /> Upload CSV
                  </button>
                  <button type="button" onClick={() => { setSheetMode(value => !value); setIsMobileMenuOpen(false) }}
                    className="flex min-h-11 w-full items-center gap-2.5 rounded-adm-sm px-3 text-left text-sm text-white/70 hover:bg-white/[0.04] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue/70">
                    <GridIcon size={16} className="shrink-0" /> {sheetMode ? 'Use product cards' : 'Use spreadsheet'}
                  </button>
                </>}
                {[
                  ['Open workflow guide', MapIcon, () => setShowWorkflowGuide(true)],
                  ['Guided tours', PlayIcon, () => setShowTourChooser(true)],
                  ['Start here', BookIcon, () => setShowStartHere(true)],
                  ['Alerts', BellIcon, () => setShowDailyTasks(true)],
                  ['Operations guide', BookIcon, () => setShowAiCopilot(true)],
                ].map(([label, Icon, open]) => <button key={label} type="button" onClick={() => { setIsMobileMenuOpen(false); open() }}
                  className="flex min-h-11 w-full items-center gap-2.5 rounded-adm-sm px-3 text-left text-sm text-white/70 hover:bg-white/[0.04] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue/70">
        ...
        <header className="hidden min-h-[72px] items-center gap-2 border-b border-adm-line bg-adm-bg px-3 py-2 lg:flex lg:gap-4 lg:px-6">
      ...
      <nav className="lg:hidden fixed bottom-0 inset-x-0 z-40 flex items-stretch border-t border-adm-line bg-adm-sunken/95 backdrop-blur-md" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
        ...
        <button onClick={() => { setMobileToolsOpen(false); setIsMobileMenuOpen(true) }}
