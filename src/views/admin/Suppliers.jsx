...
export default function Suppliers({ canCreateSupplier = false, secureMode }) {
  ...
  async function createSupplier(value, key) {
    setError(''); setNotice('')
    if (secure) {
      const response = await createSupplierBff(value, key)
      ...
      if (saveError) return { ok: false, code: 'ADMIN_SERVICE_UNAVAILABLE', error: safeUiError('SUPPLIER_SAVE_FAILED') }
    }
    setNotice('Supplier saved with the reason you entered.'); await load(); return { ok: true }
  }
  ...
  return <div className="mx-auto max-w-6xl space-y-5 pb-12">
    ...
    {supplierTab === 'directory' && (<section className="overflow-hidden rounded-adm border border-adm-line bg-adm-surface" aria-labelledby="supplier-directory-title"><div className="border-b border-adm-line bg-adm-sunken px-4 py-3"><h2 id="supplier-directory-title" className="text-sm font-semibold text-white">Supplier directory</h2></div>{loading ? <Loading /> : procurement.suppliers.length === 0 ? <Empty icon title="No suppliers recorded" body="Add a checked supplier and record where the information came from before creating a purchase order." /> : <SupplierList rows={procurement.suppliers} />}</section>)}
    {supplierTab === 'orders' && (<section className="overflow-hidden rounded-adm border border-adm-line bg-adm-surface" aria-labelledby="purchase-orders-title"><div className="border-b border-adm-line bg-adm-sunken px-4 py-3"><h2 id="purchase-orders-title" className="text-sm font-semibold text-white">Purchase orders</h2><p className="mt-1 text-xs text-white/45">Creating an order and receiving its batches are not available here yet.</p></div>{loading ? <Loading /> : procurement.purchaseOrders.length === 0 ? <Empty title="No purchase orders recorded" body="No supplier commitment has been saved yet." /> : <PurchaseOrderList rows={procurement.purchaseOrders} />}</section>)}
...
export function SupplierDialog({ value, onChange, onCancel, onSave, saving, retrySafe, returnFocusRef }) {
  const [internalValue, setInternalValue] = useState({ name: '', contactEmail: '', leadTimeDays: '14', reason: '' })
  const formValue = value || internalValue
  const isRetrySafe = retrySafe ?? true
  const { run, busy: internalSaving, uncertain, locked, error } = useRetainedFulfillmentCommand(onSave, isRetrySafe, 'supplier directory')
  const isSaving = saving ?? internalSaving
  const closeDisabled = isSaving || (uncertain && isRetrySafe)
  const closeRef = useRef(null)
  const update = (field) => (event) => {
    const next = { ...formValue, [field]: event.target.value }
    if (onChange) onChange(next)
    else setInternalValue(next)
  }
  return <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/75 sm:items-center sm:p-4" role="presentation"><AdminDialog onClose={onCancel} closeDisabled={closeDisabled} initialFocusRef={closeRef} returnFocusRef={returnFocusRef} labelledBy="supplier-dialog-title"><form aria-labelledby="supplier-dialog-title" onSubmit={async (event) => { event.preventDefault(); const result = await run({ ...formValue, leadTimeDays: Number(formValue.leadTimeDays) }); if (result?.ok) onCancel() }} className="max-h-[calc(100dvh-1.5rem)] overflow-y-auto w-full space-y-4 rounded-t-adm border border-adm-line bg-adm-surface p-5 text-white sm:max-w-md sm:rounded-adm" role="dialog" aria-modal="true"><div className="flex items-start justify-between gap-4"><div><h2 id="supplier-dialog-title" className="font-sans text-xl font-bold">Add verified supplier</h2><p className="mt-1 text-sm text-white/50">This records identity only; it does not approve pricing or create a purchase order.</p></div><button ref={closeRef} type="button" onClick={onCancel} disabled={closeDisabled} aria-label="Close supplier dialog" className="grid h-11 w-11 place-items-center rounded-adm-sm border border-adm-line">×</button></div><label className="block text-sm font-semibold text-white/70">Supplier name<input disabled={locked} className={`${INPUT} mt-1`} required minLength={2} maxLength={120} value={formValue.name} onChange={update('name')} /></label><label className="block text-sm font-semibold text-white/70">Contact email<input disabled={locked} className={`${INPUT} mt-1`} type="email" maxLength={254} value={formValue.contactEmail} onChange={update('contactEmail')} /></label><label className="block text-sm font-semibold text-white/70">Expected lead time in days<input disabled={locked} className={`${INPUT} mt-1`} type="number" min="0" max="365" required value={formValue.leadTimeDays} onChange={update('leadTimeDays')} /></label><label className="block text-sm font-semibold text-white/70">Reason and source<textarea disabled={locked} className={`${INPUT} mt-1 min-h-[88px] resize-y py-3`} required minLength={3} maxLength={500} value={formValue.reason} onChange={update('reason')} /></label><div aria-live="polite">{error && <StateBanner tone={uncertain ? 'warning' : 'danger'}>{error}</StateBanner>}</div><div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={onCancel} disabled={closeDisabled} className="min-h-[44px] rounded-adm-sm border border-adm-line px-4 font-semibold">Cancel</button><button disabled={isSaving || (uncertain && !isRetrySafe)} type="submit" className="min-h-[44px] rounded-adm-sm bg-blue px-4 font-bold text-white disabled:opacity-50">{isSaving ? 'Saving…' : uncertain ? isRetrySafe ? 'Retry same command' : 'Reconcile supplier first' : 'Save supplier'}</button></div></form></AdminDialog></div>
}
