...
export default function Suppliers({ canCreateSupplier = false, secureMode }) {
  ...
  return <div className="mx-auto max-w-6xl space-y-5 pb-12">
    ...
    {showForm && <SupplierDialog retrySafe={secure} onCancel={() => setShowForm(false)} onSave={createSupplier} />}
  </div>
}
...
export function SupplierDialog({ onCancel, onSave, retrySafe = true }) {
  const [value, setValue] = useState({ name: '', contactEmail: '', leadTimeDays: '14', reason: '' })
  const { run, busy: saving, uncertain, locked, error } = useRetainedFulfillmentCommand(onSave, retrySafe, 'supplier directory')
  const closeDisabled = saving || (uncertain && retrySafe)
