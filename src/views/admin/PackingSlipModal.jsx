        ...
        <div className="border-2 border-slate-900 rounded-xl p-5 space-y-4 font-mono text-sm bg-white">
          ...
          <div className="flex items-center justify-between border-b-2 border-slate-900 pb-3">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-bold bg-slate-900 text-white px-2 py-0.5 rounded uppercase">STANDARD DELIVERY</span>
            </div>
          ...
          <div className="text-center py-2 bg-slate-50 border border-slate-200 rounded">
            ...
            <p className="text-[10px] font-bold text-slate-500 mt-1">TRACKING BARCODE: *JT88941092PH*</p>
          </div>

          {/* Sender & Recipient Grid */}
          <div className="grid grid-cols-2 gap-4 border-t-2 border-b-2 border-slate-900 py-3">
            <div className="space-y-1">
              <p className="text-[10px] font-bold text-slate-400 uppercase">SENDER / SHIP FROM:</p>
              <p className="font-bold text-slate-900">K2 Jimzon BOS (Makati Hub)</p>
              <p className="text-slate-600 text-[11px]">Staff Custodian: {order.staff_assignee || 'Elena Guerrero'}</p>
              <p className="text-slate-500 text-[10px]">Contact: +63 917 555 0192</p>
            </div>

            <div className="space-y-1 border-l border-slate-200 pl-4">
              <p className="text-[10px] font-bold text-slate-400 uppercase">SHIP TO / RECIPIENT:</p>
              <p className="font-bold text-slate-900">{order.customer || 'Maria Santos'}</p>
              <p className="text-slate-600 text-[11px]">Address: 142 Amorsolo St, Legazpi Village, Makati City</p>
              <p className="text-slate-500 text-[10px]">Contact: +63 920 111 4455</p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between border-t-2 border-slate-900 pt-3 text-[11px]">
            <div>
              <p className="font-bold text-slate-900">PAYMENT METHOD: COD (Cash on Delivery)</p>
              <p className="text-slate-500 text-[10px]">Collect ₱{order.total || 750} from recipient upon delivery.</p>
            </div>
            <div className="text-right">
              <span className="inline-block bg-emerald-100 text-emerald-800 font-bold text-[10px] px-2.5 py-1 rounded border border-emerald-300">
