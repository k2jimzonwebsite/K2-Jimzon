            ...
            {DAILY_FLOW.map((s, i) => (
              <li key={i} className="rounded-adm-sm border border-adm-line bg-adm-surface p-3.5">
                <div className="flex items-start gap-3">
                  ...
                  {s.section && (
                    <button onClick={() => jump(s.section)} className="shrink-0 self-center rounded-adm-sm border border-adm-line bg-white/5 px-3 py-1.5 text-xs font-medium text-white/80 hover:bg-white/10 hover:text-white">
                      Open →
                    </button>
                  )}
                </div>
