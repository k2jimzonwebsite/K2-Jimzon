...
export default function AdminAuthModal({ isOpen, onClose }) {
  ...
  const field = 'w-full rounded-adm-sm border border-white/15 bg-black/30 px-4 min-h-12 py-3 text-base text-white placeholder:text-white/35 focus:border-blue focus:outline-none transition-colors'
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-md p-3 sm:p-4 overflow-y-auto animate-in fade-in duration-200">
      <div className="w-full max-w-md my-auto rounded-adm border border-adm-line bg-adm-surface p-6 sm:p-7 text-white shadow-2xl relative overflow-hidden font-sans">
        ...
        <div className="mb-7 text-center">
          <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-adm bg-blue/15 border border-blue/30 text-xl">🔒</div>
        ...
        {step === 1 ? (
          <form onSubmit={submitCredentials} className="space-y-4">
            {error && (
              <div className="p-3.5 rounded-adm-sm border border-crimson/40 bg-crimson/15 text-crimson text-sm font-bold">⚠️ {error}</div>
            )}
            ...
            <button type="submit" disabled={loading}
              className="w-full rounded-adm-sm bg-blue hover:bg-blue-deep min-h-12 py-3 text-sm font-extrabold text-white shadow-lg transition-all active:scale-[.99] disabled:opacity-50">
            ...
            <div>
              <button type="button" onClick={() => loginWithGoogle()}
                className="w-full min-h-12 py-3 px-4 rounded-adm-sm bg-white hover:bg-slate-100 text-slate-900 font-extrabold text-sm transition-all shadow-md flex items-center justify-center gap-2">
          ...
          <form onSubmit={submitCode} className="space-y-4 animate-in fade-in duration-200">
            ...
            <input type="text" inputMode="numeric" maxLength={6} value={code} autoFocus
              ...
              className="w-full text-center tracking-[0.4em] font-mono text-2xl font-extrabold rounded-adm-sm border border-blue/40 bg-black/50 px-4 py-3.5 text-blue placeholder:text-white/30 focus:border-blue focus:outline-none focus:ring-2 focus:ring-blue transition-all" />
            <div className="pt-2 flex flex-col-reverse sm:flex-row gap-2.5 sm:gap-3">
              <button type="button" onClick={() => { setStep(1); setError(''); setCode('') }}
                className="flex-1 rounded-adm-sm border border-white/20 bg-white/10 min-h-12 py-3 text-sm font-bold text-neutral-300 hover:bg-white/15 hover:text-white transition-all">
                ← Back
              </button>
