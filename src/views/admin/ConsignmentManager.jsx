...
export default function ConsignmentManager() {
  ...
  const scan = async (codeOrSku, stage, selectedItemId = null) => {
    ...
    const manifestItem = selectManifestItem({ items, code: clean, products: products || [], stage, selectedItemId })
    if (!manifestItem) {
      throw new Error(selectedItemId
        ? 'That manifest line is no longer on this manifest. Refresh and try again.'
        : `Barcode or SKU ${clean} is not on this manifest. Add the manifest line before scanning it.`)
    }
    // Re-check the chosen line against its current quantities, so a row rendered
    // before someone else's scan cannot act on stale numbers (MAP-028 H-017).
    const refusal = scanRefusalReason(manifestItem, stage)
    if (refusal) { setError(refusal); throw new Error(refusal) }
