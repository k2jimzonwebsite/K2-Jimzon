...
function EmailDraftForm() {
  ...
  const submit = (event) => {
    ...
    if (!form.email.trim() && !form.phone.trim()) {
      setError('Enter an email address or mobile number so K2 can reply.')
      return
    }
    ...
    <TuscanCard className="overflow-hidden">
      <div className="border-b border-line p-5 sm:p-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="font-serif text-xl font-semibold text-navy">Leave K2 a message</h2>
            <p className="mt-1 text-sm leading-relaxed text-navy-soft">Secure website chat is still being activated. For now, this creates a ready-to-send email in your email app.</p>
          </div>
        </div>
      </div>
      <form onSubmit={submit} className="space-y-4 p-5 sm:p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label htmlFor="contact-email" className="block text-sm font-semibold text-navy">Email
        ...
        <p className="text-xs leading-relaxed text-navy-soft">Enter at least one contact method so K2 can reply.</p>
        <label htmlFor="contact-message" className="block text-sm font-semibold text-navy">How can we help?
          <textarea id="contact-message" className="store-field mt-1.5 min-h-36 w-full resize-y px-4 py-3" value={form.message} onChange={update('message')} maxLength={2000} required placeholder="Tell us what you are looking for or what K2 should review" />
        </label>
        {error && <p role="alert" className="rounded-xl border border-crimson/25 bg-crimson/5 p-3 text-sm text-crimson">{error}</p>}
        <button type="submit" disabled={!form.name.trim() || !form.message.trim()} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-lg bg-crimson px-5 text-sm font-bold text-white transition-transform duration-150 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-45">
          Open email draft <ArrowIcon size={16} />
        </button>
        <p className="text-xs leading-relaxed text-navy-faint">Opening the draft does not send it. Review the message in your email app, then press Send.</p>
      </form>
    </TuscanCard>
  )
}
  ...
  return (
    <main className="store-section pb-24 pt-10 md:pb-20 md:pt-14">
      <div className="max-w-3xl">
        ...
        <p className="mt-4 max-w-2xl text-base leading-7 text-navy-soft">Questions about products, Pasabuy sourcing, an order, or business supply are welcome. No purchase or account is required.</p>
      </div>
      ...
      <div className="mt-9 grid items-start gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(18rem,0.85fr)] lg:gap-12">
        {secureMessaging ? (
          <div>
            ...
            <button type="button" onClick={() => go('messages')} className="mt-3 min-h-11 text-sm font-bold text-crimson underline decoration-crimson/35 underline-offset-4">Open your existing K2 messages</button>
          </div>
        ) : <EmailDraftForm />}
        ...
        <aside aria-label="K2 contact channels" className="lg:pt-2">
          ...
          <p className="mt-2 text-sm leading-6 text-navy-soft">Only confirmed public K2 details are shown here.</p>
          <dl className="mt-5 border-b border-line">
            ...
            <ContactRow label="Viber / WhatsApp"><span className="font-normal text-navy-soft">Public business number awaiting confirmation</span></ContactRow>
          </dl>

          <div className="mt-8 rounded-2xl bg-forest-wash p-5 text-forest">
            <h2 className="text-base font-bold">Live staff availability</h2>
            <p className="mt-2 text-sm leading-6">Not published yet. K2 will show staff as online only after the website can verify an active staff session; until then, use the message or email options above.</p>
          </div>
          <p className="mt-5 text-xs leading-5 text-navy-faint">Never send passwords, payment card details, or one-time codes through any contact channel.</p>
        </aside>
      </div>
    </main>
  )
