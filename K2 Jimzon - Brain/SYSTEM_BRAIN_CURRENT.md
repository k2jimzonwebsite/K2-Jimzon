...
**25 September intake cleanup candidate (IDEA-20260925-01; feature branch only):** Inventory and Sheet launch actions now start with a scan. With the production Admin BFF flag off, scan → exact product check → manual ChatGPT prompt → Smart Paste remains the operating path; Sheet's scan-to-review handoff was connected, and an exact existing barcode opens lot review. With the Admin BFF on, the same launch action opens the prepared seven-step intake session, whose production schema and server cutover remain gated by MAP-017/018/020. The old pre-scan choice modal and an unused Admin UI kit were moved to `archive/2026-09-25-intake-and-unused-ui/` with hashes and recovery copies. Earlier dated chooser and design-kit descriptions below are historical source evidence, not the current candidate behavior. No production behavior or provider state changed in this cleanup branch.
