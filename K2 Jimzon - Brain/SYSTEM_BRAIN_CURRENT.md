...
**22 September Admin quick tools (IDEA-20260922-01, locally verified, not deployed):** The gear panel now has named tools and shortcuts to existing search, barcode scan, inventory, orders, messages, Workflow map and keyboard help. Shared AdminDialog handles focus and Escape; the draggable gear recovers within the viewport after resizing. Fields and placeholders are readable, expiry guidance reflects the 90-day arrival rule without releasing stock, and scratchpad notes explicitly stay in this browser. Quick-tools and sales-planner browser cases passed; 38 focused contracts and the Admin build/security/artifact gates passed (214.07/300.00 kB minified Admin application). These are fixture/local checks, not staff acceptance or production evidence. MAP-025 retains exact-release and physical-device acceptance. Evidence, changed surfaces and recovery: `docs/evidence/20260922-admin-quick-tools/README.md`.

**22 September whole-Storefront reading and readiness preparation (IDEA-20260921-09, local only):** Shared labels/metadata use a 14px floor, fields use 16px, browser default root size is respected, light/dark accent text is separately contrasted, basket names wrap and policy paragraphs have a reading measure. Checkout and its saved note no longer advertise unapproved payment methods; wholesale copy is simpler. Fifteen customer browser checks passed, including 44 route/theme/viewport combinations, checkout and shopping/message/recovery journeys. Local inventory and payment PostgreSQL rehearsals passed; 38 focused sales/tour/release/copy contracts passed. Storefront build passes at 150.17/150.50 kB gzip JS and 29.41/30 kB CSS. No release, provider connection, real inventory approval or payment activation occurred. Evidence: `docs/evidence/20260921-storefront-readiness/README.md`. MAP-017/018/019/023/025/026 retain their production, owner-input and human-acceptance gates.

**21 September Storefront readability (IDEA-20260921-08 / MAP-027, locally verified, not deployed):** Store shelf names and product details use Source Sans UI type; shelf names are 17px, metadata 14px and chat inputs 16px. Store sheets, chat labels, controls and footer use contrasting theme colors. Mobile scene arrows are replaced by swipe/pinch guidance while category tabs remain available. Short drags change shelves, pinch preserves the existing zoom, and Ctrl/Meta-wheel remains available to the browser. Keeper Ask retains its position while focus moves to its submit button. Focused contracts pass 109/109, store browser checks 12/12 including numeric contrast, 200% root text, two-touch pinch and overlay collisions; Storefront build/security/budget gates pass (150.17/150.50 kB gzip JS, 29.25/30.00 kB gzip CSS). Evidence and recovery: `docs/evidence/20260921-store-readability/README.md`. Deployment and physical-device acceptance remain in MAP-027; no production/provider change is claimed.
