...
**25 September intake route distinction:** The older deployed scan chooser opens a prompt/Smart Paste flow, not a stock-saving automatic API command. The newer seven-step modal shares one server session for manual AI and automatic API paths and initializes it before scanning; production lacks its session table. Automatic API additionally requires the inactive Admin BFF and OWNER-007 approval. The owner reports a formerly working intake action, but the exact action and error have not been observed because computer use reports `User unavailable`. No production change was made. Evidence: `docs/evidence/20260925-inventory-readiness/README.md`.
