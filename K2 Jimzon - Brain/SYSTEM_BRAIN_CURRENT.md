...
**25 September delivery/payment current-state audit (IDEA-20260925-06):** Live checkout renders fixed region/weight delivery options and GCash/MariBank preferences. Live SQL found no Admin rate-control tables or signed delivery command; the protected rate editor is not active. The live per-order staff delivery RPC exists but does not cap a new fee against the previously accepted amount. The anonymous direct order RPC accepts a client-supplied fee from ₱0–₱100,000 as customer-confirmed without a server rate calculation. The deployed Admin direct payment call sends four arguments, while production only has the older three-argument function with no structured evidence or distinct-verifier check. No authenticated staff operation or real transfer was exercised. Local focused contracts pass 70/70; full delivery/payment acceptance is blocked in MAP-023/025. Evidence: `docs/evidence/20260925-delivery-payment-workflow-audit/README.md`.

**25 September owner delivery-policy clarification:** The owner selected J&T-based customer rates now and a separately controlled standard-rate option later. This is a policy decision, not an applied database or deployment change. The live browser still uses its fixed regional J&T-based matrix, which is not server-authoritative; the exact-locality J&T workbook remains a prepared pilot and the Admin rate editor remains inactive. No future standard-rate amount or activation has been supplied. MAP-023 records the implementation sequence and acceptance evidence.

**25 September customer-order audit (IDEA-20260925-05):** The exact live Storefront rendered catalog, cart and checkout for one Barilla item with ₱149 product + ₱95 Metro Manila delivery = ₱244; GCash/MariBank appeared, COD did not, and an empty required name blocked submission. Read-only production SQL found the current direct order RPC and three submitted order requests with payment not requested; the signed guest order RPC was absent. Local simulated selling, payment UI and focused API checks passed 13/13, 36/36 and 49/49, and guest BFF source verification passed. This does not prove a valid live order, paid transfer, staff fulfillment or buyer status continuity. A checkout payment sentence implied payment on arrival while COD is off; corrected wording is locally prepared, not deployed. MAP-023/025 retain those gaps; evidence: `docs/evidence/20260925-ordering-flow-audit/README.md`.

**25 September owner launch and inventory clarification (IDEA-20260925-04):** The owner wants both separate websites fully operational for customer and staff use, and wants Codex to perform the technical transfer of existing channel stock figures. Sister/cousin staff will check QRs and real operations; a separate payment verifier is available but the exact actors are not yet recorded. Repository inspection found only synthetic marketplace snapshot CSV fixtures, no real owner channel export. The existing Admin catalog CSV path handles product data; canonical stock requires verified receiving or opening-balance reconciliation. Fresh read-only production SQL found 30 product rows, 21 batch rows and 29 balance rows, with no `channel_shops`, `channel_shop_allocations` or `inventory_transfer_requests` tables. Multi-shop allocation/transfer SQL is locally rehearsed but absent from the live migration ledger. No channel quantities were imported, no physical count was certified, and no stock or allocation write was made from this clarification. MAP-018/023/025/026 own the next evidence.

**25 September full production readiness audit (current, IDEA-20260925-03):** K2's live Storefront catalog rendered 22 products with stock labels and the live Admin rendered staff sign-in. Supabase project `pixplcjqivlfflickobf` reported `ACTIVE_HEALTHY`; its migration ledger includes the scoped stock correction `20260925111537`, and independent SQL reconfirmed no PostgreSQL `PUBLIC` execute while named public readers remain. The latest post-stock audit is 12 critical/0 high. Vercel has separate Storefront/Admin production projects, both latest production-target deployments `READY` on documentation commit `57604cd`; the underlying source release is `f625381` with GitHub CI run `36027662677` now successful. The three owner-only Drive backup files still exist with correct ownership and folder. These checks do not establish real stock counts, buyer/staff completion, payment, account cutover, signed guest cutover, managed recovery or launch acceptance. Full status, remaining MAP owners and exact evidence limits: `docs/evidence/20260925-production-readiness/README.md`. Overall full production readiness remains **not accepted**.

**25 September MAP-017 stock grant live correction (current):** Owner authorization for the scoped change and the encrypted backup's two companion files was received. All three backup files are in the unshared owner-only K2 Production Backups Drive folder; the encrypted envelope was independently downloaded and SHA-256 matched, and both companion files were independently read back. Production migration `20260925111537` applied the exact `20260925_map017_stock_public_grant_revocation.sql` (SHA-256 `479BCCE8AFE8EEA68B7C89F85370F5EE06FC28268ADC7DF9BC155D236E88A94D`). Independent production SQL shows PostgreSQL `PUBLIC` EXECUTE absent while named `anon` and `authenticated` EXECUTE remain, owner `postgres`, and 21 stock rows. Public stock RPC returned HTTP 200/21 rows; the exact-host catalog rendered 22 products with availability. Fresh full-schema export/audit: 12 critical and 0 high, down from 13 critical. No storefront/Admin code deployment accompanied this database-only correction. The remaining six direct guest grants and six provider-owned defaults keep MAP-017 active. Earlier 25 September preparation, deferral and upload-blocked entries below are historical, superseded for current state. Evidence and recovery: `docs/evidence/20260925-map017-stock-apply/README.md` and `docs/runbooks/DATABASE_BACKUP_AND_RESTORE_RUNBOOK.md`.

**25 September MAP-017 encrypted backup upload:** After explicit owner authorization, the encrypted envelope for backup `current-pixplcjqivlfflickobf-2026-09-25T100842718Z-dde4432c4355` was uploaded to owner-only, unshared Drive file `1iPIOt0Z-sd8g4a3g4zhJj5VGgwKHOb3M` in the K2 Production Backups folder. Metadata readback showed owner `k2jimzonwebsite@gmail.com`, correct folder parent and 825,060 bytes. Independent raw download SHA-256 matched the local envelope. Automatic approval review rejected uploading its separate redacted manifest because the approval named only the encrypted file. The envelope alone does not satisfy independent offsite restore recovery; owner approval for the manifest and restore receipt is pending. No production SQL changed, and the live stock grant remains present on read-only recheck.

**25 September MAP-017 stock apply preparation after owner reopening:** The owner replied “bro fix it” after deferring, reopening the reviewed scoped stock permission correction. Read-only live ACL and migration checks still show `PUBLIC` execute plus explicit `anon` and `authenticated` execute, function owner `postgres`, 21 stock rows, and ledger ending at `20260921033348`. A public-key stock RPC returned HTTP 200 and 21 rows; the canonical catalog returned HTTP 200. New encrypted backup `current-pixplcjqivlfflickobf-2026-09-25T100842718Z-dde4432c4355` passed an isolated restore with 51 public relations; the exact prepared migration and rollback passed on that restored application schema and preserved 21 stock rows. The local restored database was removed. Automatic approval review rejected uploading the sensitive encrypted backup to the existing owner-only K2 Drive folder because the destination lacked explicit trusted user authorization. No workaround or production SQL change occurred. The owner must explicitly approve that exact backup destination before Codex can finish the recovery gate and apply the scoped change. MAP-017 and OWNER-005 hold the next action.

**25 September owner decision (MAP-017):** The owner explicitly deferred the prepared stock `PUBLIC` grant correction. Its live finding remains open; no production SQL was applied. The local migration, rehearsal, backup and restore receipt remain preparation only. Do not apply or re-request the stock change unless the owner reopens the decision. Independent guest-path preparation and Supabase provider follow-up remain under MAP-017.

**25 September MAP-017 stock permission follow-up (IDEA-20260925-02):** A fresh read-only Supabase query on project `pixplcjqivlfflickobf` returned `get_public_product_stock()` with `PUBLIC`, named `anon`, and named `authenticated` EXECUTE all present; the migration list still ends at `20260921033348`. No live permission changed. The local stock rehearsal initially failed because sandboxed `pg_ctl status` could not see its already-running practice server and a second startup could not open the log. The runner now checks `show data_directory` over localhost and refuses a different cluster. `npm run rehearse:stock-grant` exited 0 with revoke, replay, read-only check and rollback; the focused stock-grant contract suite passed 4/4; `npm run verify:development` exited 0. A fresh encrypted live application-database backup `current-pixplcjqivlfflickobf-2026-09-25T095512876Z-688de96720d7` passed an isolated restore with 51 public relations and latest migration `20260921033348` in `k2_current_restore_20260925_stock`; the dedicated restored database was then removed. Redacted local manifest and receipt are under `.tools/map017-stock-preapply-20260925.k2backup*`. This backup is local only and excludes ten managed Vault entries, Storage objects and provider settings. The production payload remains unapplied and awaits exact-payload owner authorization, controlled apply and exact-host verification under MAP-017.

**25 September human testing source release (IDEA-20260907-03/04 and IDEA-20260924-04):** Commit `f625381` is on GitHub `main`; exact-SHA Storefront/Admin Vercel deployment receipts report success and both canonical target markers returned HTTP 200. It merges seven-stage advisory intake guidance with the current barcode intake and phone navigation. Four complex dashboard views have optional Help; simple views keep visible short labels. The prepared profile, notification and account source remains gated off for production. The final local `npm run verify:release` exited 0 with 962 base tests, 41 Admin browser tests, 36 payment tests, 23 intake tests, the other listed suites and both separate builds passing. GitHub CI was still in progress at the deployment receipt; provider/database activation and real staff/customer acceptance are separate states. See `docs/evidence/20260925-human-testing-release/README.md`.

**24 September live buyer browser rehearsal (MAP-023/025):** On the exact production Storefront host, an isolated Chromium session at 390px and 1280px moved a Barilla Spaghetti listing through product, basket and checkout. Metro Manila standard/express, pickup and Visayas totals changed with the approved regional matrix; GCash and MariBank could be selected, and Cash on Delivery was absent. Missing-contact submission showed a correction message and sent no write request. Neither viewport had horizontal overflow. No valid order, payment, courier booking or delivery was made. The Barilla allergen notice repeated “Contains,” and QR payment wording was ambiguous; both are retained in MAP-018/023. Evidence: `docs/evidence/20260924-live-buyer-journey/README.md`.

**24 September local manual/automatic listing check (MAP-018):** The protected phone intake accepted both a manually pasted versioned product JSON and an automatic content result into field review, then issued one server Draft request with a server-assigned SKU in simulated browser journeys. The manual route did not start an automatic generation job. A confirmed public barcode produced an unsaved Gemini SEO suggestion for staff comparison. Staff photo upload blocked assignment until the upload resolved. The Inventory chooser now points protected Admin staff at phone intake for new products; its separate JSON action is a preview. Focused contracts, browser journeys, development verification and the Admin build passed locally. No real K2 product was created or published, and the Admin BFF is still off on the production host. Evidence and recovery are in `docs/evidence/20260924-barcode-listing/README.md`; the real acceptance remains MAP-018.

**24 September barcode-assisted listing and Gemini review (IDEA-20260924-08 / IDEA-20260924-09; local preparation):** The protected Admin route proposes Open Food Facts identity from a valid package barcode after K2's duplicate check. Step 1 can ask Gemini free tier for a small SEO draft using only freshly fetched public barcode, name, brand and quantity; it does not save product fields or send package photos. The phone intake requires staff review. It now rejects a fresh public draft if barcode, name, brand or quantity differs from the catalog item staff confirmed, and invalidates an in-flight suggestion when staff changes that decision. Staff can upload their own storefront primary, after-use and supporting photos from the existing Product Photos action after saving a Draft; intake evidence remains private and is never published automatically. Intake and Photos now explicitly say AI images are optional. The separate paid photo-grounded OpenAI intake remains inactive. An Antigravity Gemini-first photo path was removed because the owner chose free Gemini for public details only. Focused provider contracts pass 30/30, including a regression that refuses private photo dispatch to Gemini; a focused browser test passes for the catalog-change guard. A public-only provider probe exposed an invalid response-format enum (HTTP 400), which was corrected. Gemini 3.1 Flash-Lite then returned HTTP 503 twice; Gemini 3.5 Flash-Lite returned the four expected SEO fields using the local key, so the prepared route now targets 3.5. The Admin Vercel project shows a Production `GEMINI_API_KEY` Secret, and the owner reports a redeploy; this does not establish a live listing path because the Admin BFF remains off. Exact-host staff task and stock acceptance remain MAP-018/020. Design: `docs/design/BARCODE_ASSISTED_PRODUCT_LISTING.md`; evidence and recovery: `docs/evidence/20260924-barcode-listing/README.md`.

**24 September current recovery and Supabase support (MAP-017/022):** A fresh encrypted production application-database backup passed an isolated PostgreSQL restore with 51 public relations, migration ledger `20260921033348`, and a matching 14-row legacy fingerprint; ten managed Vault entries were excluded. A separate encrypted Storage backup passed local restore and checksum comparison for all 36 public `product-images` objects. Both were uploaded as eight files to the unshared, owner-only K2 Google Drive folder under `k2jimzonwebsite@gmail.com`; metadata readback matched every local size and parent. The new database envelope was independently fetched from Drive and its SHA-256 matched. The ordered MAP-019/020 migration and postflight chain then passed inside a rollback-only transaction on the restored application schema, and baseline objects and ledger were unchanged afterward. A second isolated restore included archived ACLs with non-login placeholders for missing managed role names. The same rollback-only chain passed named direct-RPC denials, preserved six signed guest grants and left the legacy chat read in place, then returned to baseline. Managed role membership, provider defaults and live preview behavior remain unproven; Storage-part download checks, managed Vault/provider recovery and owner account recovery also remain open. From the signed-in K2 Supabase dashboard, a support request for the six provider-owned `supabase_admin` default-privilege groups displayed **Support request sent**; optional project support access was off. No ticket number appeared. Supabase's answer is pending; no database grant, Auth flag or deployment changed. Exact backup IDs and restore limits are in `docs/evidence/20260924-account-migration-rehearsal/README.md` and the database recovery runbook.

**24 September account migration practice run (MAP-017/019/020; local and read-only production evidence):** Read-only production metadata shows no customer identity, guest grant, settings or notification tables; the existing eleven-argument order command has three trailing defaults, required base tables and staff helper exist, and legacy direct chat still has anonymous execute. A fresh full metadata export (99 tables, 12 views, 162 functions) compared with repository truth produced 18 findings, 13 critical and 5 high, superseding the older ten-critical count. Five authenticated functions were classified against their existing Staff or Admin/AAL2 guards and exact grants; the same export now reports 13 critical and 0 high under the reviewed repository contract. No live grant changed. The isolated account apply/rollback, behavior and replay, customer Auth rate, permission boundary, and guest moderation rehearsals passed. A stale fixed 15-route expectation in `scripts/verify-guest-bff.mjs` failed against the 17 prepared routes and was corrected to verify uniqueness and scan the two new account route files; the focused verifier and final `npm run verify:development` passed. The schema-truth focused suite passed 24/24 after updating one test for the slim active MAP. These results do not prove the full ordered chain against a restored production schema, a live backup/restore, registration, or a safe provider cutover. No production SQL or Auth setting was changed. See `docs/evidence/20260924-account-migration-rehearsal/README.md` and MAP-017/019/020/022.
**24 September follow-up:** A separate read-only MAP-017 stock ACL check now detects PostgreSQL `PUBLIC` execute while requiring intended `anon` and `authenticated` access. An attempted edit to the historical phase-one postflight altered its hash and failed the follow-up receipt gate in isolation; it was reverted. The hash-bound artifacts and production permissions remain unchanged.
**24 September cutover repair (local only):** The prepared guest cutover previously revoked only the nine-argument direct order RPC. Production's current order RPC has eleven arguments with defaults. The unapplied cutover now rejects unknown overloads and revokes both known signatures when present. An isolated PostgreSQL check denied direct order execution for `anon` and `authenticated` after cutover while preserving signed guest entry points; `npm run verify:guest-bff` and the final `npm run verify:development` passed. No production SQL, deployment or flag changed. Read-only role inspection confirmed `postgres` is not a member of `supabase_admin`; the six future-object default findings require a provider-supported role path. Evidence: `docs/evidence/20260924-account-migration-rehearsal/README.md`.

**24 September signed-in provider browser check:** Hostinger and Supabase account menus showed `k2jimzonwebsite@gmail.com`; GitHub showed the matching `k2jimzonwebsite` account. Hostinger's active domain carries the four web records in the deployment runbook. Vercel marked both separate projects Ready and the apex redirect, `www` and `admin` domains Valid Configuration. GitHub `main` showed documentation commit `2d6f7cfd9a380bce5871064de7044c28eefbe346`, with the previous code CI run for `5494b2bf71f7ecf30422e2b8c5bfc246b895bc9f` successful, but reported that `main` is unprotected. A narrow default-branch rule to block deletion and force pushes was prepared, but GitHub required email identity verification when Create was clicked; no rule was applied. Supabase showed the K2 production project Healthy and latest migration `20260921033348 admin_globe_direct_rpc`; the prepared guest/account chain is absent from its migration list. Auth Site URL and `www`, apex and Admin redirect patterns are configured; new-user sign-up is off. Its Free plan has no scheduled project backups, and point-in-time recovery is a paid add-on. The exact-host sitemap served HTTP 200 XML with Home, Catalog, Pasabuy and Trade; robots referenced it, each listed route returned 200, and sample private/product/Admin responses kept noindex. Search Console access was denied by automatic browser review pending explicit authorization, so Google ownership/submission/indexing and GA4/Cloud state remain unverified. No provider configuration was changed and no real payment, account activation or restore was verified. See `docs/evidence/20260924-provider-readiness-browser/README.md` and MAP-017/019/020/022/024/028.

**24 September documentation organization (IDEA-20260924-05):** The root MAP now holds the current agile backlog in twelve active items, each with a status, next slice, acceptance check and block. Its previous 24 September working-tree snapshot moved to `K2 Jimzon - Brain/MASTER_ACTION_PLAN_HISTORY_2026-09-24.md` with a superseded warning and original source SHA-256. That archive preserves dated audits and receipts but has no implementation authority. The active plan remains the only queue. Structural checks found all twelve MAP IDs in both files, retained sampled S/G audit IDs, and found no long dash punctuation in the touched docs; `git diff --check` passed. No application build was run for this documentation-only change. No code, schema, provider, payment or deployment state changed in this documentation move.

**24 September storefront audit repair (IDEA-20260924-04; locally prepared only):** The feature branch fixes the receipt method switch, current GCash/MariBank wording, conflicting hydrated product robots signal, and duplicate allergen prefix. It prepares optional account settings and scoped in-app notifications behind inactive account/guest BFF flags. New private settings/notification tables, signed RPCs and canonical-event triggers have passed isolated migration rehearsal; they are not applied to production Supabase. The UI offers first-use passwordless sign-in, profile editing, unread count, read action and empty-field checkout prefill only when the flags and provider boundary are active. Storefront build and bundle budget passed; focused browser/contract evidence is recorded in `docs/evidence/20260924-storefront-audit-fixes/README.md`. No live user account, notification delivery, real payment, search indexing or deployment is claimed. MAP-019/020/023/024/025/028 own cutover and acceptance.

**24 September release receipt update:** GitHub CI run `35955278280` passed both `build-and-smoke` and `critical-sql-behavior` for release SHA `5494b2bf71f7ecf30422e2b8c5bfc246b895bc9f`. GitHub production deployment receipts `6630128794` (Storefront) and `6630134932` (Admin) both report success. The canonical Storefront home, Pasabuy page, sitemap, two QR assets and Admin entry returned HTTP 200 with expected HTML, XML and PNG content types. The local aggregate `verify:release` was interrupted after its base suite emitted 947 passing results; remote CI success and live content checks complete the narrow promotion receipt, while the local runner issue remains MAP-028 work. No real purchase or payment was made. See `docs/runbooks/DEPLOYMENT_RUNBOOK.md`.

**24 September Admin phone navigation and text layout (IDEA-20260924-03; code promoted):** Commit `5494b2bf71f7ecf30422e2b8c5bfc246b895bc9f` is on GitHub `main`. The phone drawer groups work areas and staff tools; shared headings, metric details, status text and buttons wrap. Live `admin.k2jimzon.com` served `Admin-DnjF-ocB.js`, which contains the new Buying & shipments navigation label, and its target marker returned HTTP 200 with `admin`. Local evidence remains: 360px fixture walk passed all 18 sections; focused Admin contracts passed 41/41; `npm run verify:development` and `npm run build:admin` passed at 217.34/300.00 kB. The earlier full Admin browser run reported 38/39, and the corrected selector passed 1/1 in isolation. The owner-requested `npm run verify:release` did not complete; see the deployment runbook. Physical staff acceptance, authenticated work and device checks remain open in MAP-025. No provider or database state changed. See `docs/evidence/20260924-admin-mobile/README.md`.

**24 September search readiness (IDEA-20260924-02; code promoted, indexing unverified):** Catalog, Pasabuy and Trade have distinct titles, descriptions, canonicals and social text in route navigation and initial-response HTML; the Storefront build emits those pages and the four-route canonical sitemap. Live `www.k2jimzon.com/pasabuy` served the new title and description. The read-only product projection carries description fields and reports missing descriptions. Product `noindex` and sitemap exclusion remain ON pending genuine photos, unique descriptions and owner publication review. Focused contracts passed 14/14; local development and Storefront builds passed before promotion. The owner-requested aggregate release gate did not complete. No Search Console indexing, rankings or product result is claimed. MAP-024/MAP-018/MAP-025 own the remaining actions. See `docs/evidence/20260924-seo-readiness/README.md`.

**24 September manual QR choice (IDEA-20260924-01; code promoted, payment not verified):** Storefront checkout offers GCash or MariBank, records the selected method in the order note and displays the matching receiving QR after order submission. Both QR image URLs returned HTTP 200, and live `Checkout-D3LnWQvx.js` contains the MariBank option. The receipt still requires staff to confirm stock and the exact total before transfer. Local synthetic checkout and payment-boundary evidence is in `docs/evidence/20260924-manual-qr-payment/README.md`; it does not establish a real order or transfer. No funds were sent or marked paid. Second-device QR scan, recipient confirmation, real stock/total, Admin evidence entry, independent account verification and staff acceptance remain MAP-023/MAP-025.

**25 September Admin Inbox Categorization, Viewport Maximization, Message Deletion Discovery, and External Channel Readiness (IDEA-20260925-07, MAP-028 / MAP-020, locally verified):**
Delivered full inquiry categorization, channel origin detection, viewport enlargement toggle (+200px reading space), collapsible workflow sidebar, emergency message deletion guidance, external channel portal launchers, and 12px typography floor compliance in Admin BOS (`Inbox.jsx` & `adminInboxNormalization.js`):
1. **Inquiry Purpose Categorization & Channel Origin Detection:** Implemented `inferConversationCategory` and `inferConversationOrigin` in `src/lib/adminInboxNormalization.js`. Classifies messages into 5 business purposes: Wholesale / Bulk (`wholesale`), Pasabuy / Sourcing (`pasabuy`), Order & Delivery Support (`order`), Product & Shelf inquiries (`product`), and General (`general`). Detects channel origins (Website Storefront, Virtual Store Shelf, Messenger, WhatsApp, Shopee, Lazada, TikTok, Viber, Instagram).
2. **External Channels Readied (Messenger, WhatsApp, Shopee, Lazada, TikTok):** Added `getChannelPortalUrl` producing direct 1-click external portal launchers (`Open Meta Inbox`, `Open WhatsApp Web` with prefilled phone query, `Open Shopee Webchat`, `Open Lazada IM`, `Open TikTok Shop Chat`) in the active chat header alongside channel-specific instructions. Maintained K2 canonical truth: external channels do not claim live webhooks; replies are drafted or templated in Admin and copied via "Copy for external reply" for staff pasting into seller portals.
3. **Queue Filtering & Status Pills:** Added quick category filter tabs (`All`, `Wholesale`, `Pasabuy`, `Orders`, `Shelf`, `General`) with live counts, channel origin selector, and category badges rendered on each conversation list item.
4. **Viewport Full-Height Maximization (Eliminating Void Space):** Replaced fixed `h-[calc(100dvh-310px)]` container with dynamic `flex-1 min-h-0 flex flex-col overflow-hidden` across `Admin.jsx` and `Inbox.jsx`, filling 100% of the viewport height and eliminating the dead black void at the bottom of the screen. Added "Enlarge workspace" toggle that collapses the tall 6-card MetricRail into a compact 1-line KPI strip (`Active`, `Live web`, `Unread`, `Overdue`, `Wholesale`, `Pasabuy`) for maximum message reading canvas.
5. **Collapsible Workflow Sidebar:** Added sidebar toggle in chat header (`isWorkflowOpen`), allowing staff on xl displays to collapse the 320px workflow panel and expand the chat thread to the full available right width.
6. **Direct 1-Click Message Deletion & 1-Click Thread Archiving:** Added interactive "Delete" action on every message bubble (both customer inquiries and staff responses) opening an accessible `<AdminDialog>` with reason tracking ("Spam or trolling inquiry", "Inappropriate or abusive language", etc.), instant in-memory thread update, and database deletion via `database.from('messages').delete().eq('id', id)` or runtime `deleteMessage`. Added prominent 1-click **Archive / Remove Thread** button (`<CheckIcon />`, `min-h-11`) in conversation header opening `<AdminDialog>`, allowing staff to mark a conversation `Resolved` in Supabase and remove it from the active queue with optional message clearing. Reorganized "Message Actions & Deletion" guidance modal to place 1-click thread archiving and message deletion at the top for staff, demoting the raw SQL snippet (`DELETE FROM public.messages WHERE conversation_id = ... AND content = '...'`) as an advanced DBA fallback.
7. **Design & Product Register Compliance:** Enforced the strict 12px font floor across all compact badges and indicators (replacing legacy `text-[10px]` and `text-[11px]` with `text-xs`), touch targets >= 44px (`min-h-11`), satisfying `tests/admin-dialog-contract.spec.js`.
8. **Verification Evidence:** `tests/inbox-categorization-viewport.spec.js` passed 9/9 tests in 2.1m; `tests/inbox-phase2.spec.js` passed 32/32 tests in 2.8m; `tests/admin-dialog-contract.spec.js` passed 7/7 tests in 3.4s; `tests/inbox-command-retry.spec.js` (8/8 passed); `tests/inbox-read-completeness.spec.js` (7/7 passed); `npm run test:contracts` (716/716 contract tests passed + 13/13 selling-surfaces passed); `npm run verify:development` (1,646 files checked, 0 secrets, 0 boundary gaps, import-integrity green).
