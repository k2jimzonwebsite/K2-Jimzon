...
**25 September 06:05 UTC intake follow-up:** The owner reports the production Admin browser is signed in, but Codex computer use currently returns `User unavailable`, so AAL2 and staff actions are not independently observed. Fresh production Supabase reads show migration ledger latest `20260921033348`, no `product_intake_sessions`, and no three MAP-018 intake command functions. The full read-only MAP-018 preflight executed without error. No production grant, migration, row, or Storage object changed; MAP-017 remains the activation gate. Evidence: `docs/evidence/20260925-inventory-readiness/README.md`.

**25 September intake logic audit and owner correction:** The rulebook's user-facing flow scans first, checks identity and package evidence, then offers manual ChatGPT or optional paid API content, with separate human review, Draft, stock and publication commands. The newer seven-step modal follows that visible order; it initializes its resumable session when opened, but production lacks the session table. The older chooser instead labels an existing-SKU barcode prompt as “automatic” intake and new-product Smart Paste as “manual”; its scan action does not save stock. Paid API requires the inactive Admin BFF and OWNER-007 approval. The owner reports no specific failed control or error. The agent's prior claim that a formerly working workflow had broken was unsupported; existing intake behavior remains unverified because computer use reports `User unavailable`. No production change was made. Evidence: `docs/evidence/20260925-inventory-readiness/README.md`.

**25 September current-schema local rehearsal (MAP-017/018):** The fresh encrypted production database archive was restored a second time with archived ACLs to an isolated local PostgreSQL database. The prepared stock `PUBLIC` revoke passed on that clone, preserved explicit caller grants and 21-row stock-view reads for `anon` and `authenticated`, removed inherited execute for an unrelated role, and the paired recovery restored the original ACL and 51-relation/ledger baseline. A separate restored clone passed the MAP-018 preflight, full intake migration, cleanup-boundary migration and postflight in order. Local managed-role placeholders and excluded Vault entries limit this proof. No production migration, grant, Storage object or inventory row changed; live Admin MFA and physical-stock acceptance remain open. Details: `docs/evidence/20260925-inventory-readiness/README.md`.

**25 September full live schema audit (MAP-017, read-only):** The current Supabase structural export and repository auditor report 13 critical and 0 high findings: one extra stock-function `PUBLIC` grant, six transitional anonymous RPC grants, and six provider-owned `supabase_admin` default-privilege groups. The export is metadata-only and the report is a classification, not an applied correction or runtime permission test. The signed guest migration and Supabase-supported default-privilege answer remain required. Evidence: `docs/evidence/20260925-inventory-readiness/README.md`.

**25 September inventory readiness (live read-only and offsite recovery):** Production Supabase project `pixplcjqivlfflickobf` still grants PostgreSQL `PUBLIC` execute on `get_public_product_stock()` alongside explicit caller grants. `product_intake_sessions` and the draft, first-inventory and publication server functions are absent, so the prepared MAP-018 new-product intake is not live. Owner Google sign-in on the production Admin host reached the six-digit authenticator challenge; staff workflow acceptance is pending completion in the browser. Fresh encrypted database and Storage snapshots passed isolated local restores; eight encrypted/redacted files were uploaded to the owner-only Drive folder with exact sizes and owner-only permissions. All eight passed independent retrieval and SHA-256 comparison, including reassembly of the two encrypted Storage parts. Real managed-role/provider recovery, production migration/apply and real product listing remain open. No production data, grants, schema or Storage objects were changed. Evidence: `docs/evidence/20260925-inventory-readiness/README.md`; owners: MAP-017 and MAP-018.

**25 September MAP-024 Vercel Preview target correction (provider applied):** In the owner-authenticated Vercel browser, `k2-jimzon` project `prj_ULQ5zbR7zDaFCMlXVjlrZxj9sXsL` received Preview-only `K2_DEPLOYMENT_TARGET=storefront`; `k2-jimzon-admin` project `prj_hPWQKCjIQRuKB3LLlbCmlGNHjL3x` received Preview-only `K2_DEPLOYMENT_TARGET=admin`. The existing Production entries were left in place. Each project redeployed PR #13 commit `fde7e20`; Storefront deployment `2N9GPYtJmEGERnLJNiwiSEvKLhSh` and Admin deployment `Ftn3upkDGgFsZWyuSUi1HLPTq2PE` report Ready. Their exact Preview URLs rendered the Storefront home and Admin sign-in. This verifies separate Preview artifact entrypoints, not authenticated flows, stock ACL production apply, or Production release. Evidence: `docs/evidence/20260925-map017-stock-grant/README.md`.

**25 September MAP-017 stock ACL correction (local preparation):** The reviewed 24 September production export shows `public.get_public_product_stock()` with explicit `anon` and `authenticated` execute plus an extra PostgreSQL `PUBLIC` execute grant. A separate transactional SQL correction now revokes only `PUBLIC`; its companion recovery script restores that grant. The existing portable PostgreSQL suite recreates the broad grant, proves anonymous/authenticated stock-view access and another explicit function grant survive, proves an unrelated role loses inherited execute, then rolls back and checks the baseline. It also rejects correction when `PUBLIC` is already absent or a required explicit caller grant is missing. The isolated rehearsal exited 0; no production SQL or flag changed. Draft PR #13's initial Vercel Preview attempts failed at config compilation because `K2_DEPLOYMENT_TARGET` was absent in Preview; the MAP-024 provider correction and successful exact-SHA Preview receipts are recorded above. Current-schema/provider-role, exact-host and live postflight evidence remain MAP-017 work. Evidence: `docs/evidence/20260925-map017-stock-grant/README.md`.
