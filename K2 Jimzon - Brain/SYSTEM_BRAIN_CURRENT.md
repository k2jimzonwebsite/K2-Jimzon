...
**23 September 3D Store Ergonomics & Bottom Popups (IDEA-20260923-02, locally verified):** Refactored the 3D Interactive Store (`/store`) per owner request: (1) K2 Shopkeeper repositioned from top-left to a toggleable pop-up at the bottom of the screen (`.k2-store-guide` with `flex-direction: column-reverse !important;` and bottom anchor); (2) Selected product details converted from permanent sidebar column into a toggleable bottom pop-up card (`.k2-store-product-popup`) anchored above the bottom rail, dismissing cleanly when toggled or closed; (3) Floating zoom buttons (+, restore, -) removed from the 3D scene, relying on intuitive touch drag/pinch and mouse wheel/drag; (4) Eliminated bottom washed-out white gradient haze on `.k2-store-rail`, allowing the natural wood background (`/wood-bg.jpg`) to remain fully visible; (5) Removed redundant cabinet product lists, establishing the bottom rail as the single canonical source for items available in cabinets; (6) Solved mobile 375px collision prevention: `.k2-store-basket-dock` constrained with `max-width: calc(100% - 5.5rem);` to maintain clean >8px separation with closed keeper toggle, and dynamically floats to `top: 4.25rem` when keeper dialogue is open to prevent bounding box intersection while keeping 'Review basket' clickable and in-viewport. Verified with `npm run test:store-orientation` (12/12 passed including orientation, readability, and overlay collision), `tests/map027-store-polish.spec.js` (77/77 passed), `tests/mobile-store-moderation-pwa-contract.spec.js` (4/4 passed), `npm run verify:development` (zero warnings/errors), and production builds (`build:storefront` JS 150.14 kB / 150.50 kB gzip, CSS 29.42 kB / 30.00 kB gzip; `build:admin` 214.07 kB / 300.00 kB).
