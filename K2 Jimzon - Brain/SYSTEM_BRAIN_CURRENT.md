...
4. **Viewport Maximization (+200px Reading Canvas):** Added "Enlarge workspace" toggle that collapses the tall 6-card MetricRail into a compact 1-line KPI strip (`Active`, `Live web`, `Unread`, `Overdue`, `Wholesale`, `Pasabuy`) and expands main inbox height to `h-[calc(100dvh-200px)] min-h-[660px]` (from `h-[calc(100dvh-390px)]`), immediately revealing 7–10 messages without scrolling.
...
6. **Direct Message Deletion & Retraction Control:** Added interactive "Delete" action on every message bubble (both customer inquiries and staff responses) opening an accessible `<AdminDialog>` with reason tracking ("Spam or trolling inquiry", "Inappropriate or abusive language", "Errant or accidental message", etc.), instant in-memory thread update, and database deletion via `database.from('messages').delete().eq('id', id)` or runtime `deleteMessage`. Added accessible "Message Actions & Deletion" header trigger and per-staff-message "Retract" action providing an AdminDialog explaining the append-only audit trail rules and a prefilled, copy-pasteable Supabase SQL snippet (`DELETE FROM public.messages WHERE conversation_id = ... AND content = '...'`) for authorized owner manual execution.
7. **Design & Product Register Compliance:** Enforced the strict 12px font floor across all compact badges and indicators (replacing legacy `text-[10px]` and `text-[11px]` with `text-xs`), satisfying `tests/admin-dialog-contract.spec.js`.
8. **Verification Evidence:** `tests/inbox-categorization-viewport.spec.js` passed 8/8 tests in 2.0m; `tests/admin-dialog-contract.spec.js` passed 7/7 tests in 3.4s; `tests/inbox-command-retry.spec.js` (8/8 passed); `tests/inbox-read-completeness.spec.js` (7/7 passed); `npm run test:contracts` (716/716 contract tests passed + 13/13 selling-surfaces passed); `npm run verify:development` (1,646 files checked, 0 secrets, 0 boundary gaps, import-integrity green).
