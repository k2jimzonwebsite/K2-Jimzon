...
**IDEA-20260925-01 — accepted into MAP-018 and MAP-021: one scan-first intake and reversible legacy cleanup.** The owner wants the active code and documentation to follow one product-intake lifecycle, with obsolete code kept in a repository archive for recovery rather than deleted. Audit: the older Inventory chooser, scan-to-prompt modal and Smart Paste remain imported in Inventory and Sheet while the canonical seven-step intake handles scan, evidence, manual or optional paid API content, reviewed Draft, stock and publication. The old chooser's “automatic” restock wording points to prompt preparation, not an inventory write. Decision: preserve the complete legacy sources and recovery map in a dated archive; remove proven obsolete entry points from active code only after dependency and behavior checks; keep tests and docs aligned with the one canonical flow. Do not activate the production intake schema or paid API as a side effect. MAP-018 owns workflow alignment, MAP-021 owns import/build cleanliness, and MAP-017/020/OWNER-007 retain their existing activation gates. Exact live staff behavior has not been observed, so this cleanup stays on a feature branch until release evidence is ready.
