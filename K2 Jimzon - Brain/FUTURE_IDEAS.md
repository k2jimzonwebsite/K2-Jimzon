...
**IDEA-20260925-07: Admin Inbox Categorization, Viewport Maximization, Message Deletion Discovery, and External Channel Readiness. Accepted and merged into MAP-028 / MAP-020.**
Owner request and evidence: Owner uploaded screenshot from `admin.k2jimzon.com/admin-portal-k2-secure?section=inbox` showing an inappropriate staff message ("tirahin kita eh") sent to a customer asking for product taste ("patikim"), and requested: (1) how to locate and delete an errant message in Admin; (2) logic to categorize messages by origin (Storefront, Virtual Store, Messenger, WhatsApp, Shopee, etc.) and purpose/intent (Wholesale/Bulk, Pasabuy/Sourcing, Order & Delivery Support, Product & Shelf, General); (3) enlarge the inbox reading area and viewport so significantly more messages can be read at once without constant scrolling; (4) ready external channel integrations (Messenger, WhatsApp, Shopee, Lazada, TikTok) and verify all existing capabilities remain fully working. Follow-up owner request: eliminate large empty space below the chat container, and make deleting/archiving easy without technical SQL instructions ("what is this deleting messege should be easy, if we cant deelete maybe deleting in admin space will do make it archive by makign this only seenable in supabase").
Audit gate:
1. Real problem and evidence: Message deletion does not exist in browser client due to append-only audit trail rules and RLS policies on `public.messages`. Anonymous chat deletion was written in migration `20260922_anonymous_chat_moderation.sql` but gated behind inactive Admin BFF (`VITE_ADMIN_BFF_ENABLED=false`). The UI lacked any retraction guidance or SQL copy generator for errant staff messages. Furthermore, vertical screen real estate was severely cramped (`h-[calc(100dvh-390px)]`) due to stacked intro text and 6-card MetricRail, limiting visible messages to 3 bubbles, while the right workflow aside consumed 320px permanently on xl displays. External channel messages had no direct portal launchers or clear instructions for staff copy-and-paste replies. Additionally, on taller viewports, fixed `h-[calc(100dvh-310px)]` left an empty black void at the bottom of the screen.
2. Outcome: (a) Added direct message deletion with `<AdminDialog>` confirmation modal across both customer and staff bubbles with reason capture ("Spam or trolling inquiry", "Inappropriate or abusive language", etc.), immediate in-memory pruning, and database deletion via `database.from('messages').delete().eq('id', id)` or runtime `deleteMessage`; (b) Dedicated "Message Actions & Deletion" guidance modal in the chat header with 1-click archiving card at the top, clear guidance, and prefilled copy-pasteable SQL snippet (`DELETE FROM public.messages WHERE conversation_id = ... AND content = '...'`) demoted as advanced DBA fallback; (c) Channel origin inference (`inferConversationOrigin`) and inquiry purpose classification (`inferConversationCategory`) for Wholesale, Pasabuy, Orders, Product/Shelf, and General inquiries with quick filter pills, counts, and badges; (d) Viewport full-height expansion: converted Admin section container and Inbox root to `flex-1 min-h-0 flex flex-col overflow-hidden`, completely eliminating bottom dead space on all display sizes while maintaining collapsible aside; (e) 1-Click "Archive / Remove Thread" button in conversation header with `<AdminDialog>` confirmation, updating Supabase `conversations.status = 'Resolved'` and removing thread from active queue, with option to clear messages; (f) `getChannelPortalUrl` and 1-click external portal launchers (`Open Meta Inbox`, `Open WhatsApp Web`, `Open Shopee Webchat`, `Open Lazada IM`, `Open TikTok Shop Chat`) with copy-for-reply instructions; (g) Enforced 12px font floor and >=44px touch targets across all admin controls.
3. Dependencies: None. Client-side normalization and UI presentation; uses existing schema and preserves all Phase 2 contract tests.
4. Records/state/permissions/recovery: Untouched. Database RLS audit protection preserved. Authenticated admins have DELETE permission on public.messages. Verified with 9/9 tests passing in `tests/inbox-categorization-viewport.spec.js`, 32/32 tests passing in `tests/inbox-phase2.spec.js`, 7/7 tests passing in `tests/admin-dialog-contract.spec.js`, 729/729 passing contract tests, and 100% clean `npm run verify:development`.

**IDEA-20260925-04: Two-site operating launch and assisted channel-stock transfer. Accepted and merged into MAP-018/023/025/026.** The owner clarified that launch means both separate websites fully operational: customer Storefront and staff Admin. Existing marketplace channels have stock numbers; the owner wants Codex to handle the technical comparison/upload rather than make the owner manually enter rows. The owner's sister and cousins will perform staff-side QR and real-world checks, with distinct payment evidence and account verifiers available. Audit: the repository has local product CSV import and shop-allocation code, but product import cannot create stock; physical stock requires approved receiving or opening-balance reconciliation. The prepared multi-shop allocation/transfer migration is absent from the current production migration ledger. No actual channel stock export is present in the repository; only synthetic fixture CSVs exist. Decision: request the actual source export, map product/shop identities and reconcile it against current canonical lots and physical counts before any stock write. Keep physical stock, channel offer amounts, and payment confirmation separate. Codex owns the upload/rehearsal when the source and required verification exist. No production quantity or channel allocation changed in this intake.

**IDEA-20260925-03: Durable full-production readiness receipt. Accepted and merged into MAP-017 through MAP-028, chiefly MAP-025/028; documentation-only.** The owner asked to document the stock correction and check complete production readiness so later work does not depend on chat history. Audit: live sites and the scoped stock correction are verified, but signed guest/account activation, 12 schema findings, real stock/payment/staff acceptance and whole-service recovery remain open. Decision: preserve a dated cross-MAP evidence matrix in `docs/evidence/20260925-production-readiness/README.md`, refresh the current source/deployment/CI receipts, and keep all remaining work in the owning MAP items. This creates no second backlog and authorizes no additional production change.
...
**IDEA-20260925-02 outcome, 25 September:** The owner authorized the encrypted backup and both redacted companion uploads to the named owner-only Drive folder. All three were independently read back; the envelope hash matched. The reviewed stock-grant migration applied to production as ledger `20260925111537`. Live ACL, public RPC and rendered catalog checks passed; the fresh audit reduced critical findings from 13 to 12. The scoped stock work is complete; the remaining MAP-017 guest and provider findings stay active. See `docs/evidence/20260925-map017-stock-apply/README.md`.
...
**Captured:** 2026-09-25
**Raised by:** Owner (requesting location of message deletion, categorization by channel origin and purpose like Messenger, Wholesale, Pasabuy, and maximizing screen space so more messages are readable)
**Problem observed:** Admin Unified message control lacks channel/origin filtering and intent/purpose categorization (e.g. Wholesale, Pasabuy, Order, Shelf questions). The message view is vertically cramped by large KPI cards and permanently occupied by a 320px workflow sidebar, limiting message viewing density. Message deletion is not visible in direct Supabase mode, lacks single-message retraction guidance, and causes confusion when staff post unintended messages.
**Desired outcome:**
1. Clear discovery and status for conversation deletion and single-message retraction in accordance with MAP-020 audit rules.
2. Dual categorization: Origin (Website, Virtual Store, Messenger, WhatsApp, Shopee, etc.) and Purpose (Wholesale, Pasabuy, Order, Product & Shelf, General) with quick filter tabs and distinct badges.
3. Maximized viewport layout: collapsible metrics/compact mode, collapsible workflow drawer to widen chat, and tightened message bubbles displaying 2-3x more messages simultaneously.
**Evidence or example:** Owner screenshot showing "Unified message control" with 4 active threads and accidental staff message; `src/views/admin/Inbox.jsx`, `src/lib/adminInboxNormalization.js`.
**Known dependency:** MAP-020 (Admin BFF and moderation RPC migration for live conversation deletion), MAP-028 (Admin UI/UX and workflow clarity).
**Possible overlap with current behavior/MAP item:** Merged into MAP-020 (moderation boundary) and MAP-028 (Admin UI/UX and workflow clarity); no competing backlog.
**Owner decision potentially required:** Confirm whether single-message retraction/deletion policy should be added to the database in a future migration or if conversation-level moderation suffices.
