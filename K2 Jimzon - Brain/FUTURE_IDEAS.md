...
**IDEA-20260924-09: Public-only Gemini SEO suggestion. Merged into MAP-018.** After staff confirm a fresh public barcode match, the protected Admin route may send only public barcode, name, brand and quantity to Gemini free tier. It returns unsaved copy for staff review. No package photos or internal K2 data go to the free tier. Paid photo-grounded generation remains inactive under OWNER-007; the Admin BFF activation remains MAP-020 work.

**IDEA-20260924-08: Barcode-assisted new-product listing. Merged into MAP-018.** Staff scan a package barcode, compare a public grocery identity suggestion, capture private package evidence, then review manual product JSON or an enabled paid content result before a server-assigned Draft. Staff can upload their own storefront photos after Draft; AI images are optional. Stock, price and publication remain separate approvals. Public lookup has a manual fallback. The source is prepared locally, with live staff acceptance still open.
