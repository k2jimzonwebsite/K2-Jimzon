...
Owner request and evidence: Owner uploaded screenshot from `admin.k2jimzon.com/admin-portal-k2-secure?section=inbox` showing an inappropriate staff message ("tirahin kita eh") sent to a customer asking for product taste ("patikim"), and requested: (1) how to locate and delete an errant message in Admin; (2) logic to categorize messages by origin (Storefront, Virtual Store, Messenger, WhatsApp, Shopee, etc.) and purpose/intent (Wholesale/Bulk, Pasabuy/Sourcing, Order & Delivery Support, Product & Shelf, General); (3) enlarge the inbox reading area and viewport so significantly more messages can be read at once without constant scrolling; (4) ready external channel integrations (Messenger, WhatsApp, Shopee, Lazada, TikTok) and verify all existing capabilities remain fully working.
Audit gate:
1. Real problem and evidence: Message deletion does not exist in browser client due to append-only audit trail rules and RLS policies on `public.messages`. Anonymous chat deletion was written in migration `20260922_anonymous_chat_moderation.sql` but gated behind inactive Admin BFF (`VITE_ADMIN_BFF_ENABLED=false`). The UI lacked any retraction guidance or SQL copy generator for errant staff messages. Furthermore, vertical screen real estate was severely cramped (`h-[calc(100dvh-390px)]`) due to stacked intro text and 6-card MetricRail, limiting visible messages to 3 bubbles, while the right workflow aside consumed 320px permanently on xl displays. External channel messages had no direct portal launchers or clear instructions for staff copy-and-paste replies.
2. Outcome: (a) Added direct message deletion with `<AdminDialog>` confirmation modal across both customer and staff bubbles with reason capture ("Spam or trolling inquiry", "Inappropriate or abusive language", etc.), immediate in-memory pruning, and database deletion via `database.from('messages').delete().eq('id', id)` or runtime `deleteMessage`; (b) Dedicated "Message Actions & Deletion" guidance modal in the chat header providing immediate, prefilled copy-pasteable SQL snippet (`DELETE FROM public.messages WHERE conversation_id = ... AND content = '...'`) for authorized owner Supabase execution, plus anonymous conversation moderation status; (c) Channel origin inference (`inferConversationOrigin`) and inquiry purpose classification (`inferConversationCategory`) for Wholesale, Pasabuy, Orders, Product/Shelf, and General inquiries with quick filter pills, counts, and badges; (d) "Enlarge workspace" toggle expanding container height to `h-[calc(100dvh-200px)] min-h-[660px]` with compact KPI strip (+200px vertical space, 7-10 messages visible), collapsible right workflow sidebar, and tightened message stream spacing; (e) `getChannelPortalUrl` and 1-click external portal launchers (`Open Meta Inbox`, `Open WhatsApp Web`, `Open Shopee Webchat`, `Open Lazada IM`, `Open TikTok Shop Chat`) with copy-for-reply instructions; (f) Enforced 12px font floor across all admin badges.
...
4. Records/state/permissions/recovery: Untouched. Database RLS audit protection preserved. Authenticated admins have DELETE permission on public.messages. Verified with 8/8 tests passing in `tests/inbox-categorization-viewport.spec.js`, 7/7 tests passing in `tests/admin-dialog-contract.spec.js`, 716/716 passing in `npm run test:contracts`, and 100% clean `npm run verify:development`.
