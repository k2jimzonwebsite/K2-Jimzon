...
**MAP-017 public stock function grant rule (25 September; prepared):** `public.get_public_product_stock()` must retain explicit `EXECUTE` for `anon` and `authenticated` while PostgreSQL `PUBLIC` has none. Its stock view remains readable by `anon` without exposing private batch rows. The scoped correction may change only that function's `PUBLIC` execute grant; it must fail if the function, the observed broad grant, or either required explicit caller grant differs from the reviewed preflight. Apply only after a fresh verified backup, current ACL review and controlled change gate. The recovery SQL restores the broad grant only when reversing this exact correction; neither SQL file is live merely because it is committed.
