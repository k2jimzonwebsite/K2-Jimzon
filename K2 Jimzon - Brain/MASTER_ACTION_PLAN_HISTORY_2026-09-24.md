# Master Action Plan history through 24 September 2026

This is a superseded historical snapshot, not an active backlog. The current work queue is `MASTER_ACTION_PLAN.md`. Use this file to trace dated audit findings, earlier states, decisions, commands, and evidence without treating an old pending line as a current task. Source working-tree SHA-256 before punctuation cleanup: `B180E82AC9A3DD90DF77E51E7AD2A8D33DD755FDF8AB9CFF9D95C2844BF35CCB`.

The wording below is preserved from the earlier plan except that long dash characters were replaced with plain hyphens for readability.

---

# K2 Jimzon Master Action Plan

**24 September scoped release receipt:** Code SHA `5494b2bf71f7ecf30422e2b8c5bfc246b895bc9f` has green GitHub CI run `35955278280` and successful separate Storefront/Admin production deployment receipts `6630128794`/`6630134932`. Canonical Storefront/Admin routes and the QR assets returned HTTP 200. The local aggregate `verify:release` did not complete because its runner stalled after 947 passing base cases; keep that runner issue in MAP-028. MAP-023 still owns an actual confirmed payment and MAP-025 owns representative staff/customer phone acceptance. Product indexing remains gated. See `docs/runbooks/DEPLOYMENT_RUNBOOK.md`.

**24 September current-queue reconciliation:** GCash and MariBank receiving choices and QR assets are supplied and deployed; do not request the QR asset again. MAP-023 still owns a second-device recipient check, final stock/total confirmation, one real transfer with structured evidence, independent merchant-account verification, and payment exception/reconciliation acceptance. MAP-025 still owns real staff/customer phone acceptance. The 13 September MAP-017 follow-up `20260909023000` is applied and independently verified: its fresh audit reduced critical findings from 26 to 10. Do not reapply phase one or that follow-up. MAP-028 retains the incomplete local aggregate runner and all unproven provider/real-workflow gates. Earlier dated snapshots below remain history; this paragraph and the owning current sections supersede their old pending-asset, 26-finding, and missing-deployment statements.

**24 September owner-independent audit of this queue:** Reviewed the current MAP-017 post-apply audit and the existing deployment/payment receipts. The remaining four anonymous guest/coupon RPC grants depend on a working signed guest replacement before revocation; six `supabase_admin` default-privilege findings require a supported provider-authorized correction. Payment acceptance needs the owner's recipient check and distinct real staff/account verifier; product publication needs reviewed physical facts and media rights. The local QR/SEO/Admin code is already promoted, so there is no second code promotion to perform for those choices. A single focused `npm run evidence:map024-discovery` attempt could not fetch `/` in this restricted environment (`MAP024_DISCOVERY_REFUSAL: GET / failed (network-error)`); it creates no new live discovery evidence and must not close G-011. No broad test suite, provider write, payment, or deployment was performed. Exact next engineering action: prepare/rehearse the signed guest prerequisite and provider-supported default-privilege correction against the current schema, then capture target-host and real-role evidence during the recorded change window. Recovery for this documentation-only reconciliation is to revert this MAP diff; no operational state changed.

**22 September implementation slice  -  IDEA-20260922-03 through -06 (MAP-019/020/024/027/028; active, locally prepared).** The mobile Store Shopkeeper is now a 56×56 collapsed control, auto-collapses behind every sheet, and has a horizontal 44px camera toolbar. The footer has the six exact owner-approved destinations without connector claims. The separate Admin build emits a network-only service worker and browser-provided install action. Prepared moderation uses separate Admin/SuperAdmin delete, block and manual-unblock commands, private keyed 32-byte IP hashes, active-account deletion refusal and content-free receipts; BFF routes and Inbox controls are wired. Evidence: focused red then 4/4 green, Storefront/Admin production builds green, rendered 390×844 verification measured 56×56 with the guide still collapsed behind the Questions sheet, and `npm run verify:development` passed after the final code edit. Not applied or live: Supabase migration, Vercel deployments, real-role moderation, install prompt on a supported device, and physical Android/iOS acceptance. Recovery: revert this scoped slice; after an authorized database application, use `20260922_anonymous_chat_moderation_rollback.sql` only inside the MAP-017 change window. Exact next action: wait for owner authorization for the database/deployment change window, then perform real-role and physical staff/customer acceptance.

**23 September branch review/cutover correction (same active slice).** Review found and local code now addresses append-only conversation-event cascades, orphan guest-grant scopes, a null-role metadata disclosure, three rejected verifier actions, the legacy direct-chat block bypass, protection for every account-linked customer, and stale open store-chat state after a deletion. The isolated PostgreSQL 17.11 rehearsal now passes migration application, missing-role denial, delete with event history/grant cleanup, block/unblock, revoked-account deletion refusal, and rollback; it caught two SQL syntax defects before merge. The moderation migration revokes the legacy `submit_storefront_chat_v1` client grant: applying it while `VITE_GUEST_BFF_ENABLED=false` breaks existing direct chat. Therefore this migration is **prepared only** and must be coupled to a tested signed guest/Admin BFF cutover in a controlled change window; no production database apply, flag flip, or deployment is implied by merging code. Next: complete release gate, then preview/real-role acceptance and coordinated provider cutover. If the provider cutover cannot be made safely, leave the migration unapplied and direct chat unchanged. Recovery: use the reviewed rollback under MAP-017 backup/change-window controls and restore the prior Storefront/Admin artifacts.

**23 September local verification status:** The focused moderation/mobile/PWA contracts, isolated PostgreSQL rehearsal, `npm run verify:development`, and both target-specific production builds passed before the final landscape correction. The latest focused MAP-027/moderation/security suite passes 85/85 after a red-to-green test for collapsed Shopkeeper stacking under a landscape chat sheet, correction of stale source assertions, and prepared-route documentation counts (Admin 95, Storefront 15). A fresh `npm run verify:release` printed all 945 base cases as passing but again hung without a phase summary and was interrupted; later suites and current builds were not reached. The aggregate release gate is not passed. Read-only production Supabase inspection found no `start_guest_conversation_v1` or moderation RPC, while anonymous still has the direct `submit_storefront_chat_v1` grant. The prepared migration unconditionally renames that missing guest RPC and therefore cannot apply to today's production schema; even after prerequisite reconciliation, its direct-grant revocation requires a coordinated signed guest BFF cutover. No push, migration, flag flip, or deploy is authorized by this evidence. Next: reconcile the guest-RPC prerequisite with the live schema under MAP-019/020, diagnose the base-suite teardown, complete the entire release gate and real-role/landscape acceptance, then use the MAP-017 controlled cutover and rollback procedure. Keep current live direct chat unchanged until then.
...
**23 September release-gate correction (latest local evidence; MAP-019/020/025/027/028):** The prior apparent Playwright base-suite hang was Windows sandbox denial during web-server teardown; running the browser gate with permitted local process control exited normally. Two stale/slow UI fixtures were corrected (Store reset accessible name and isolated Admin HelpTip mount), and the complete multi-stage owner-close phone test was given a 240-second budget after it passed in 1.8 minutes. Final `npm run verify:release` exited 0: all 1,152 tests passed, followed by separate Storefront and Admin production builds, source/security boundary checks, secret scans, and bundle budgets. This supersedes the unfinished-run statement above for **local code verification only**. The Vercel connector can now list both K2 projects; its latest production deployments still point to old `origin/main` SHA `3f018432d786f2f9b15d9f6d80d68ba4a2a40522`. Live Supabase still lacks the guest-start/reply RPC and tables needed by the prepared moderation migration, while direct chat remains granted. Do not apply that migration or claim delete/IP block live from this green build. Remaining work: reconcile and rehearse the signed guest BFF/database prerequisites under MAP-019/020, obtain the owner-controlled MAP-017 cutover decision, verify preview and real Admin/SuperAdmin/guest behavior plus physical phone acceptance, then coordinate GitHub/Vercel/database promotion or hold the whole release. No push, production database write, flag flip, or deployment was performed in this recheck. Recovery before production application: revert the scoped local slice; after application use the reviewed moderation rollback only within the MAP-017 backup/change window and redeploy the prior artifacts.

**23 September guest prerequisite investigation (same active MAP-019/020 cutover):** Read-only production migration ledger lists no MAP-019 identity or MAP-020 guest-submission migration. Production has only `submit_storefront_chat_v1`, not the signed guest start/reply functions, `customer_accounts`, guest grants, guest request secrets/nonces/rates or conversation receipts; the direct writer remains active. A second preflight defect was reproduced: the guest boundary checked only the old nine-argument `submit_order_request_v2` identity, while live production has the newer eleven-argument function with three trailing defaults (so a nine-argument call is valid). A new isolated current-signature fixture first failed the preflight with `MAP020_PREFLIGHT: live guest source commands are missing`; the preflight now accepts the exact eleven-argument function only if at least the required two trailing defaults exist. The isolated preflight plus moderation apply/behavior/rollback rehearsal then passed. This is **local compatibility preparation**, not a production apply or proof that the full guest/Admin cutover is ready. The production prerequisites, matching private/Vercel signing secrets, Turnstile setup, preview denial/continuity tests, Admin secure-mode dependencies, owner-approved backup/change window, and real-host role/guest/phone acceptance are still open. Do not push the full bundle as a live delete/block release until these are proven; keep direct chat unchanged. Next: reconcile the exact MAP-019/020 migration chain against current live schema in a rollback-only rehearsal, inventory both Vercel environments without exposing values, then obtain explicit authorization for the coordinated database and two-artifact cutover. Recovery is to leave the current live artifacts/flags untouched until the rehearsal and approval gates pass.
...
- `npm run prebuild` clean (1407 files, 0 leaks, 0 boundary gaps, 16 expected anon grants intact).
- `npm run test:contracts` 657/657 PASS plus `test:selling-surfaces` 8/8 PASS (665 total green).
- `npm run build:storefront` passed at JS 150.15 kB / 150.50 kB gzip and CSS 29.25 kB / 30.00 kB gzip; `npm run build:admin` passed at 198.22 kB / 300.00 kB minified.
- `npm run rehearse:shop-transfers` PASSED (MAP-026 allocation, custody, fail-closed, rollback); `npm run rehearse:purchase-hold` 48/48 held (MAP-023, unlisted ordering with rollback included). Nothing applied to production.
- GlobeOverlay DB-field binding fix (`src/components/globe/GlobeOverlay.jsx:137-148`, MAP-027, still uncommitted): diff-inspected, binds `text/name/channel` with legacy fallbacks, no layout/token/typography change. Coverage gap recorded: no spec pins the overlay field binding, so the fix is covered by inspection plus full-suite green only; a dedicated overlay-binding contract remains open under MAP-027.
- Working tree still holds exactly 2 uncommitted files (`GlobeOverlay.jsx` + System Brain entry). Live DB still holds 0 `reviews` rows and the Admin BFF env gate is still off, so Admin saves and published-review display remain pending owner-authorized activation and real-host verification. This entry is local verification evidence only, not a live claim.

**17 September Full Admin BOS Mobile Experience, Touch Ergonomics, Responsive Readability, and Operational Logic Hardening (IDEA-20260917-08, MAP-028 M, code locally verified):**
Completed exhaustive audit and surgical hardening across all 48 Admin BOS views, navigation structures, and operational modals:
- **Admin BOS Mobile Usability & Readability Remediations Applied:**
  - *M-01 (Resolved):* Upgraded section tabs in `DeliveryRateControl.jsx` from `min-h-9` to `min-h-11` (44px) with thumb-zone spacing.
  - *M-02 (Resolved):* Standardized `MobileScannerModal.jsx` interactive triggers (Close, Sync, +1 Scan) and quick-tap SKU tiles to `min-h-[44px]` touch targets.
  - *M-03 (Resolved):* Standardized `MilanPackingScannerModal.jsx` Close and Done Packing buttons to `min-h-[44px]`.
  - *M-04 (Resolved):* Upgraded `AdminAuthModal.jsx` dismiss button from `py-1` to `min-h-11 flex items-center justify-center`.
  - *M-05 (Resolved):* Standardized `AdminAiCopilotModal.jsx` close button from `h-10 w-10` to `min-h-11 min-w-11` (44x44px).
  - *M-06 (Resolved):* Upgraded manual SKU/barcode input in `MobileScannerModal.jsx` to `text-base sm:text-sm min-h-[44px]`, eliminating iOS Safari auto-zoom.
  - *M-07 (Resolved):* Added `min-w-[720px]` to 9-column allocation matrix table in `ShopAllocationManager.jsx`, preventing column collapse on mobile screens.
  - *M-08 (Resolved):* Wrapped `RebalanceModal` and `CreateTransferModal` in `ShopAllocationManager.jsx` with `AdminDialog` focus trap and upgraded to `max-h-[calc(100dvh-1.5rem)]`.
  - *M-09 (Resolved):* Added mobile-responsive layout bounds to `AdminToolsWidget.jsx` dialog (`fixed inset-x-2 bottom-20 max-h-[calc(100dvh-6rem)] sm:absolute`), preventing floating widget viewport spillage on mobile devices.
  - *M-10 (Resolved):* Added `overflow-x-auto scrollbar-none` to sub-navigation tabs in `JntVipDispatchModal.jsx`, preventing horizontal page blowout.
  - *M-11 (Resolved):* Replaced raw em dashes across staff copy in `Admin.jsx`, `AdminToolsWidget.jsx`, `AdminAiCopilotModal.jsx`, `AdminAuthModal.jsx`, `AutomaticIntakePanel.jsx`, `BulkCsvImportModal.jsx`, `DeleteProductsModal.jsx`, `DeliveryRateControl.jsx`, `Inbox.jsx`, `InventoryGrid.jsx`, `Overview.jsx`, and `adminGuide.js`, and synchronously updated `tests/unlisted-product-ordering-contract.spec.js`.
- **Verification Evidence:** Full contract suite passes; selling surfaces pass; `npm run prebuild` clean (0 leaks, 0 boundary gaps); separate Storefront build <= 150.50 kB gzip; Admin build <= 300.00 kB minified. Ready for staging and promotion to `main`.

**17 September Full-Surface Mobile Experience, Responsive Readability, and Operational Logic Remediation (IDEA-20260917-07, MAP-028 L, code locally verified):**
Completed full-surface mobile experience audit and executed verified surgical remediations across Storefront and Admin BOS:
- **Mobile Usability & Readability Remediations Applied:**
  - *L-01 (Resolved):* Standardized chat inputs and textarea in `StoreChatPanel.jsx` to 16px (`text-base` / `.store-field`) with `min-h-[44px]`, eliminating iOS Safari auto-zoom on input focus.
  - *L-02 (Resolved):* Upgraded `StoreChatPanel.jsx:53` header text to `text-[12px]`, strictly adhering to the repository >= 12px font floor.
  - *L-03 (Resolved):* Replaced customer-facing em dashes with hyphens and colons across `Pasabuy.jsx:125-126`, `Wholesale.jsx:138`, `CatalogGrid.jsx:56, 105`, and `DeliveryEstimate.jsx:131`.
  - *L-04 (Resolved):* Formatted `<CartDrawer />` single-line Suspense mount in `StorefrontApp.jsx:64`, bringing `tests/storefront-discovery-contract.spec.js` to 16/16 PASS and full contracts to 653/653 PASS (100% green).
  - *L-05 (Resolved):* Added direct order inquiry CTA on `Confirmation.jsx` (`<CrimsonButton onClick={() => openStoreChat(...)}>Chat with staff about this order</CrimsonButton>`) when running in direct Supabase mode, giving customers instant staff access for their new order reference.
  - *L-06 (Verified):* Re-confirmed touch targets >= 44x44px (`min-h-11`), viewport stability (`min-h-[100dvh]`), bottom navbar spacer (`h-16 md:hidden`), floating concierge clearance (`bottom-20 right-5`), and input retention across validation retries.
- **Verification Evidence:** 653/653 contract tests PASS; 8/8 selling surfaces PASS; `npm run prebuild` clean across 1,400 files (0 leaks, 0 boundary gaps); separate Storefront build passed at JS 150.15 kB / 150.50 kB gzip and CSS 29.43 kB / 30.00 kB gzip; Admin build passed at 196.06 kB / 300.00 kB minified. Ready for staging and promotion to `main`.

**17 September J&T VIP Mobile & Desktop Courier Booking, Template Parity & Interactive Guide Suite (IDEA-20260917-02, IDEA-20260917-03, MAP-023, code locally verified):**
Completed pure J&T VIP bulk CSV calculation engine, official Excel template parity (`exptemplete_en.xls`), 1-order batch CSV export, interactive step-by-step dispatch walkthrough mode, mobile and desktop courier dispatch assistant modal, single-warehouse website fulfillment boundary, and storefront trust badges:
- **Official J&T VIP Template Parity & Bulk Engine (`src/lib/jntVipBulkEngine.js`):** Audited official `exptemplete_en.xls` template and verified exact 13 contractual columns: `Receiver(*)`, `Receiver Telephone (*)`, `Receiver Address (*)`, `Receiver Province (*)`, `Receiver City (*)`, `Receiver Region (*)`, `Express Type (*)`, `Parcel Name (*)`, `Weight (kg)  (*)`, `Total parcels(*)`, `Parcel Value (Insurance Fee) (*)`, `COD (PHP) (*)`, `Remarks`. Accommodated the double space in column 8 (`Weight (kg)  (*)`). Added `generateJntVipSingleOrderCsv(order)` to export a single order in official bulk batch format, enabling zero-typing 1-click batch upload on J&T VIP (`My Order > Create Waybills In Bulk`) for single packages. Standardizes Philippine mobile numbers (`normalizePhilippinePhone`) into canonical `09xxxxxxxxx` format. Formats smart recognition addresses (`formatJntSmartAddress`) enabling 1-click paste into J&T VIP single waybill address parser. Generates RFC 4180 compliant CSV batches with UTF-8 Byte Order Mark (`\uFEFF`) and robust escaping for quotes, commas, and newlines. Validates J&T tracking numbers (`validateJntTrackingNumber`) accepting official `PH...` barcodes and 10 to 16 digit numerical codes.
- **Interactive Step-by-Step Guide & Modal UX (`src/views/admin/JntVipDispatchModal.jsx` & `OmniOperationsHub.jsx`):** Mounted in `OmniOperationsHub.jsx` for single orders ("J&T VIP Book" in Confirmation Queue and Packing Station) and batch exports ("J&T VIP Batch (.csv)" action header). Includes an interactive `[Step-by-Step Guide]` mode with a 3-step progress stepper:
  - *Step 1 (Recipient Address):* 1-tap copy of smart recognition address token, direct J&T VIP portal launcher, clear step instructions for staff/sister, and 1-order batch CSV download fallback.
  - *Step 2 (Package & COD Specs):* Dedicated 1-tap copy chips for Weight, Declared Value, COD Amount, and Remarks reference.
  - *Step 3 (Waybill Handover):* Barcode scanner input and save action marking order as dispatched and handed over.
  Standard overview mode retained with quick 1-order batch CSV download and guide toggle. Full `<AdminDialog>` compliance, $\ge 44\times 44$px touch targets (`min-h-11`), strictly $\ge 12$px typography floor, zero em dashes, and zero raw emojis (clean SVG icons only).
- **Storefront Single-Warehouse Brand & Trust Boundary (`Checkout.jsx` & `CartDrawer.jsx`):** Customer sees "Fulfilled by K2 Jimzon (Manila Hub Dispatch)" across Checkout delivery option cards and "Fulfilled by K2 Jimzon · Manila Warehouse Dispatch" in the Cart Drawer. Respects single master warehouse boundary (`MANILA_MAIN` backs the website while marketplace channels manage separate warehouse reservations).
- **Verification Evidence:** Playwright contract test suites `tests/jnt-vip-dispatch-contract.spec.js` (9/9 PASS) and `tests/admin-dialog-contract.spec.js` (7/7 PASS) passed (16/16 PASS total); `npm run prebuild` clean (0 secrets, 0 boundary gaps across 1,396 files); Admin BOS bundle passed at 196.06 kB / 300.00 kB minified (103.94 kB headroom); Storefront bundle passed at JS 149.89 kB / 150.50 kB gzip, CSS 29.37 kB / 30.00 kB gzip.

**17 September Multi-Shop Channel Allocation & Custody Transfer Engine (IDEA-20260917-01, MAP-026, code locally verified):**
Completed pure multi-shop inventory projection calculation engine, physical custody transfer state machine, PostgreSQL schema and stored procedures, migration rehearsal harness, and Admin BOS staff interfaces:
- **Multi-Shop Channel Allocation Engine (`src/lib/channelAllocationEngine.js`):** Pure calculation engine implementing owner-confirmed multi-shop inventory projection rules. Enforces default target coverage of 2 sellable units per active individual shop account (`DEFAULT_TARGET_UNITS = 2`, `DEFAULT_SHOPS` for Shopee Main/Outlet, TikTok Main/Live Outlet, Lazada Flagship/Express). Evaluates explicit statuses (`Covered`, `Thin`, `Skipped`, `Out`, `Needs review`). Guaranteed zero double-counting invariant: sum of shop allocations never exceeds Master warehouse stock. Computes outbox sync deltas (`calculateOutboxSyncDeltas`) for connector diff updates.
- **Physical Custody Transfer State Machine (`src/lib/custodyTransferEngine.js`):** Implements physical stock movement rules under staff-request -> admin-approval -> receiver-acceptance workflow (`pending_approval`, `approved`, `rejected`, `in_transit`, `completed`, `cancelled`). Fails closed on insufficient unreserved lot stock. Enforces admin-only review role permissions (`UNAUTHORIZED_TRANSFER_REVIEW`) and mandatory rejection reasons.
- **Database Schema, Views & Stored Procedures (`supabase/migrations/20260917_multi_shop_allocation_and_transfers.sql` and rollback):** Created tables `public.channel_shop_allocations` and `public.inventory_transfer_requests`, stock projection view `v_multi_shop_stock_projection`, and stored procedures `rebalance_shop_allocations_v1`, `request_inventory_transfer`, `review_inventory_transfer`. Safe role exception handlers (`exception when undefined_object then null`) ensure seamless execution across blank PostgreSQL instances.
- **Local PostgreSQL 17 Migration Rehearsal (`scripts/rehearse-multi-shop-transfers-portable.mjs`):** Dedicated test harness on port 54334 verifying migration application, idempotent replay, ample stock distribution (12 units across 6 shops -> all Covered), scarce stock distribution (3 units -> Priority 1 Covered, Priority 2 Thin, Priority 3-6 Out), fail-closed over-quantity request rejection, admin approval transitions, and clean rollback (`npm run rehearse:shop-transfers` exit code 0).
- **Admin BOS Staff Interface (`src/views/admin/ShopAllocationManager.jsx` & `ChannelIntegrations.jsx`):** Sub-navigation rail in Channel Integrations toggles between "Channel readiness & connectors" and "Multi-shop stock allocation & custody". Features Allocation Matrix view with real-time summary cards, product-by-product breakdown, per-shop status badges, and interactive "Rebalance Projections" modal with live 2-unit preview and scarcity warnings. Features Custody Transfers view with pending approvals queue for admins with Approve / Reject dialogs, historical transfers log, and "Request Custody Transfer" staff modal. Complies strictly with 12px font floor, `min-h-11` touch targets, SVG icons, and zero raw emojis.
- **Local Verification Evidence:** Playwright contract suite `tests/multi-shop-allocation-contract.spec.js` passed 4/4 tests; `npm run rehearse:shop-transfers` passed exit 0; `npm run prebuild` clean across 1,393 files with 0 leaks and 0 boundary gaps; `npm run build:admin` passed at 196.06 kB / 300.00 kB minified (103.94 kB headroom); `npm run build:storefront` passed at JS 149.89 kB / 150.50 kB gzip, CSS 29.32 kB / 30.00 kB gzip.
...
- **Store Overlay Collision Test Isolation & Mobile Layout Clearance:** Added `'store-overlay-collision.spec.js'` to `testIgnore` in `playwright.config.js` and asserted its inclusion in `tests/release-ci-contract.spec.js`. Added responsive rules in `src/interactive-store.css` under `@media (max-width: 900px)` for `.k2-store-flat-scene` and `.k2-store-flat-scene-card`, ensuring 50px vertical clearance above the basket dock and resolving cross-platform font metric variance in Playwright.
- **PostgreSQL 17 Linux Socket Permissions, Port Deconfliction & Clean Shutdown:** Added socket permissions step in `.github/workflows/ci.yml`, added `-k "${dataDir}"` socket flags in rehearsal scripts, inspected log files on startup error, and retained `{ stdio: 'ignore' }` to prevent Windows `spawnSync` hangs. Replaced hardcoded `pg_ctl.exe` in `finally` blocks with `executable['pg_ctl.exe']` to guarantee clean shutdown on Linux, and changed `rehearse-final-admin-concurrency.mjs` port to dedicated `54332`.
- **Local Verification Evidence:** `npm run rehearse:purchase-hold` (48/48 properties held); `npm run rehearse:payment-recovery` (exit 0); `npm run rehearse:final-admin` (exit 0); `npm run test:store-orientation` (5/5 passed in 3.1m); `tests/release-ci-contract.spec.js` (9/9 passed); `npm run prebuild` clean (1386 files, 0 leaks, 0 boundary gaps); `npm run build:storefront` (JS 149.88 kB / 150.50 kB gzip; CSS 29.26 kB / 30.00 kB gzip); `npm run build:admin` (196.06 kB / 300.00 kB minified).
...
- **Inventory Intake Chooser Modal (`AddInventoryChooserModal.jsx`):** Mounted a high-contrast chooser modal triggered whenever staff clicks `+ Add Inventory` across the top header toolbar or `InventoryGrid` intro actions. Asks whether the intake is:
  1. *Automatically via Barcode:* Restocking incoming physical units for existing master catalog SKUs via EAN-13 laser barcode scan and FEFO expiry lot binning. Offers 1-click direct scan (`Scan & Intake Now →`) or interactive guided walkthrough (`Start Guided Tour (5 steps)`).
  2. *Manually via ChatGPT Studio:* Onboarding brand-new Italian provisions without an existing SKU. Offers 1-click Smart Paste JSON launcher, direct manual form creation, or interactive guided walkthrough.
- **Top Header Action & Grid Workspace Integration (`Admin.jsx` & `InventoryGrid.jsx`):**
  - Added primary `+ Add Inventory` button (`bg-blue font-bold min-h-11`) to the top header toolbar when `section === 'inventory'` and wired `launchInventoryTool('add-inventory')`.
  - Mounted primary `+ Add Inventory` button in `WorkspaceIntro` actions (`data-tour="add-inventory-btn"`).
  - Forwarded `onStartTour={handleStartTour}` prop into `<InventoryGrid />` so tours launch smoothly from the chooser modal.
- **Staff Quick Tools Widget Polish (`AdminToolsWidget.jsx`):**
...
- **Contract Testing & Verification:**
  - Expanded `tests/spotlight-tour-contract.spec.js` (10/10 PASS).
  - Strict $\ge 12$px font floor, $\ge 44\times 44$px touch targets (`min-h-11`), and anti-emoji policy 100% verified.
  - Full prebuild passed (1386 files checked, 0 secrets, 0 boundary gaps).
  - Admin bundle passed at 195.99 kB / 300.00 kB minified.
  - Storefront bundle passed at 149.89 kB / 150.50 kB gzip.
...
- **Interactive Tour Engine (`SpotlightTourOverlay.jsx`):** Renders SVG spotlight mask cutout over target bounding rect (`getBoundingClientRect`), pulsating radar dot with glowing ring, floating high-contrast popover card with viewport boundary clamping and auto-scroll, step indicator dots, previous/next buttons, and full keyboard navigation (`Esc`, `[N]`, `[P]`).
- **Interactive ChatGPT Studio Station (`tourData.js` & `SpotlightTourOverlay.jsx`):** Built-in prompt studio station for Step 3 of Manual Product Intake. Features category selector pills (Dolci, Pasta, Caffè, Olio), optional custom Italian product title override, live formatted prompt preview, and 1-click clipboard copying (`Copied to Clipboard!`) to streamline AI JSON spec drafting for Smart Paste.
- **Workflow Selection Modal (`TourSelectionModal.jsx`):** Modal dialog with dual workflow cards ("Manual Product Intake with ChatGPT Studio" vs "Automatic Stock Intake & Barcode Scan"), highlighting step counts, estimated duration, key features, and 1-click tour launch triggers.
- **Master Workflow Graph & Guide Modal Integration (`MasterWorkflowGraph.jsx`, `WorkflowGuideModal.jsx`, `Admin.jsx`):** Mounted `[Start Interactive Spotlight Tour]` button in the Mission header and in `WorkflowDetailDrawer.jsx`, forwarded `onStartTour` through `WorkflowGuideModal`, added `[Guided Tours]` button to the Admin desktop header, and replaced legacy `<span>🗺️</span>` with clean SVG `<MapIcon size={16} />`.
- **InventoryGrid Tour Target Anchors (`InventoryGrid.jsx`):** Anchored `data-tour="inventory-actions"`, `data-tour="scan-box-btn"`, `data-tour="smart-paste-btn"`, `data-tour="add-product-btn"`, and `data-tour="search-input"`.
- **Contracts, Invariants & Verification:** Contract test `tests/spotlight-tour-contract.spec.js` passes 8/8; full contract suite passes 652/652; selling surfaces pass 8/8; `npm run prebuild` clean with 0 leaks (1385 files); Admin bundle passes at 192.41 kB / 300.00 kB minified (107.59 kB headroom); Storefront bundle passes at 149.90 kB / 150.50 kB gzip. 100% compliant with strict $\ge 12$px typography floor, minimum $44\times 44$px touch targets (`min-h-11`), and anti-emoji policy.
...
- **Canvas Auto-Focus & Visual Type Filtering (`WorkflowSvgCanvas.jsx`):** Implemented smooth viewport auto-focus centering on the active node (`focusNode`) with a dedicated "Focus Active Node" toolbar button; added interactive step-type filter pills (`All types`, `Scans`, `Decisions`, `Actions`, `Committed`) with opacity dimming for non-matching nodes; added pulsating active ring (`animate-ping`) and high-contrast focus styling; upgraded badge typography to comply strictly with the 12px font floor.
- **Staff Role Filtering & Keyboard Navigation (`MasterWorkflowGraph.jsx`):** Added a dedicated staff role dropdown selector (`All Roles`, `Milan Courier`, `Manila WH Admin`, `Owner / Admin`, `Inventory Specialist`, etc.) to isolate responsibilities across the operations graph; added keyboard listeners (`[N]` or `ArrowRight` for next step, `[P]` or `ArrowLeft` for previous step) with visual kbd shortcut badges in the header bar.
- **Visual Breadcrumb Flow & 1-Click Clipboard Copy (`WorkflowDetailDrawer.jsx`):** Added a responsive 3-step sequence flow banner (`Prev Step` → `Current Active Step` → `Next Step`) enabling instant step transitions; implemented 1-click clipboard copy buttons with visual feedback (`Copied!`) for the exact Admin click target, SOP physical directive, and test simulation barcode; sanitized all grounding evidence text to comply with the 12px typography floor.
- **Verification & Budgets:** All 10 workflow tests passed (`tests/workflow-guide-truth.spec.js`, `tests/workflow-graph-canvas.spec.js`); full contract suite passed (652/652 PASS); selling surfaces passed (8/8 PASS); prebuild security passed with 0 leaks and 0 gaps (1378 files); Admin bundle passed at 191.12 kB / 300.00 kB minified; Storefront passed at 149.89 kB / 150.50 kB gzip.
...
- **100% J&T VIP Parity without Manual Typing:** Aligned data models with J&T VIP official 13-column bulk template (`exptemplete_en.xls`) and Smart Recognition text box (`[Name], [Phone], [Street Address], [Barangay], [City], [Province]`). Added `extractJntOrderDetails` and `generateJntVipSingleOrderCsv` in `src/lib/jntVipBulkEngine.js` for 1-click single or bulk order upload directly into J&T VIP portal. Embedded payment method in customer notes and payment metadata, automatically populating J&T column 12 (`COD (PHP) (*)`) with exact grand total for COD or ₱0.00 for prepaid.
- **Admin BOS Step-by-Step Interactive Guide:** Added `[Step-by-Step Guide]` toggle in `JntVipDispatchModal.jsx` with an interactive 3-step progress stepper guiding staff through recipient address auto-recognition, package weight & declared value chips, and barcode scanner handover.
- **Evidence & Budgets:** Playwright contract suites (`tests/jnt-vip-dispatch-contract.spec.js`, `tests/storefront-recovery-ui.spec.js`, `tests/storefront-truth-contract.spec.js`, `tests/operations-hardening.spec.js`) 46/46 PASS; `npm run prebuild:storefront` clean (0 leaks, 0 boundary gaps); Admin BOS bundle passes at 196.06 kB / 300.00 kB minified; Storefront bundle passes at 149.89 kB / 150.50 kB gzip.
...
**24 September phone-first Admin follow-up (IDEA-20260924-03; code promoted, staff acceptance pending).** Owner requested clearer staff words, less box nesting, grouped phone navigation and removal of overflowing or oversized text across the Admin. The phone drawer groups work areas and Staff tools; the active area opens first. The short mobile title, visible Inventory action and wrapping shared containers are live. Local evidence: all 18 registered sections stayed within a 360px viewport; 41 focused contracts, `npm run verify:development` and Admin build passed (217.34/300.00 kB entry). The complete Admin browser run reported 38/39, with its sole failure from a selector matching a hidden desktop heading; the corrected case passed alone (1/1). Commit `5494b2bf71f7ecf30422e2b8c5bfc246b895bc9f` is on GitHub `main`; the live Admin marker and `Admin-DnjF-ocB.js` feature label were confirmed. The aggregate `verify:release` did not complete; see the deployment runbook. Pending: representative Staff/Admin task timing and physical-device acceptance under MAP-025. Roll back with `git revert 5494b2b` and promote that commit; no database/provider action. Evidence: `docs/evidence/20260924-admin-mobile/README.md`.
...
**ACTIVE 18 September  -  Admin palette declutter, slice 1 (IDEA-20260918-01, row 2 above).**
**Remaining acceptance, IDEA-20260920-14 / I-012:** representative staff must
...
Owner moved this from idea intake to active work: declutter Admin BOS now, change the palette if needed, remove no logic. Presentation-only; all routes, states, permissions, and copy meaning unchanged.
- Shared `HelpTip` hover/focus ? replaces permanent paragraphs in `WorkspaceIntro`, `SectionHeading`, and the Admin desktop header subtitle (`HelpTip.jsx`, `AdminWorkspaceUi.jsx`, `Admin.jsx`).
- One accent rule: blue reserved for primary actions and selection. Neutralized decorative sky/amber/purple/gold on Start Here banner and diagram pills (`StartHereGuide.jsx`), Workflow Map and Guided Tours toolbar buttons (`Admin.jsx`), fulfillment map toggle (`OmniOperationsHub.jsx`), EDITABLE badge (`ScanToAiModal.jsx`), receiving flow map button and kickers (`ConsignmentManager.jsx`), section labels and the banned 4px side-stripe (`InventoryGrid.jsx`), domain pills/table headers/lens labels and Phone Intake (`Sheet.jsx`), section headings (`StaffPermissionManager.jsx`, `CouponManager.jsx`, `SmartPasteModal.jsx`), and copilot quote/message boxes (`AdminAiCopilotModal.jsx`).
- Primary actions unified to blue: Create coupon (`CouponManager.jsx`), Save new quote version (`PasabuyManager.jsx`), Phone Intake and AI Spec Enricher (`Sheet.jsx`), Enrich product specs (`InventoryGrid.jsx`).
- Deliberately untouched: all StateBanner/StatusPill/metric/priority tones, Draft/expiry/fee badges, GlobeCms stars, role badges, the internally single-hue intake-modal and calculator themes, and print styles. Color stays supplemental to text/icons throughout.
- Verification: `npm run prebuild` clean; `test:admin-ui` 31/33 with 2 failures reproduced on the unmodified baseline (stale em-dash expectations, KPI fixture); `build:admin` 197.46 kB / 300 kB. Slice 2 (remaining decorative hues, if any) only after owner reviews slice 1 on a running Admin.
- Humanizer pass, same date (humanizer skill, technical-reference voice): rewrote the 18 section descriptions (`Admin.jsx` SECTIONS), 7 widget descriptions (`dashboardWidgets.js`), and 7 Start Here steps (`adminGuide.js` DAILY_FLOW) in plain staff words. Active voice, no em dashes, no AI vocabulary; every operational fact kept (verified-paid is not cash in hand, finalize-once rule, exact-scan wording). Labels, titles, and test-pinned strings untouched.
- Money-lens merger, same date (one widget, three lenses): the Shop & channel metrics, Sales & records, and Revenue trend widgets shared one order/product pipeline and one period, so they now render as a single bordered panel in `Overview.jsx` with a Channels | Sales | Revenue lens switcher (segmented control on desktop, dropdown select on phones). Widget ids, sidebar labels, headings, aria-labels, drill-downs, filters, buckets, empty/loading/error paths, and unavailable handling are byte-identical; only the `hidden` driver moved from widget to lens. PanelHeading descriptions across all widgets collapsed behind the shared hover-? marker.
- Continuity bar, same date: the period pills, Refresh, updated/snapshot status, and three live totals (verified value, unread, open Pasabuy, all from existing reads) now sit in one sticky strip, so period and snapshot travel with staff down the page. Command-center intro paragraph collapsed behind ?.
- Scroll-truth slice, same date (widget clicks reset scroll; guide overlays disfigured background scroll): section switches now move heading focus with `preventScroll`, so clicking a left-panel widget or bottom tab no longer yanks the scroll position. New shared `useBodyScrollLock` freezes background scroll while WorkflowGuideModal, TourSelectionModal, StartHereGuide, and every AdminDialog modal is open (nested-safe save/restore). SpotlightTourOverlay keeps the live UI interactive by design and now scrolls a target into view only when it is actually outside the viewport, instantly instead of smooth. Verification: all 42 guide/tour/dialog contracts pass (workflow-guide-truth, workflow-graph-canvas, staff-workflow-guide-contract, spotlight-tour 12/12, admin-dialog 7/7, jnt-vip-dispatch 9/9); `test:admin-ui` 31/33 with the same 2 pre-existing baseline failures; `build:admin` 197.70 kB / 300 kB.
- Full read-only deep-dive audit, same date (documentation only, no code changed): all 47 Admin BOS files read in 4 lanes; findings in `docs/evidence/20260918-admin-bos-deep-dive/README.md`. Headline results: 3 scanner implementations should collapse under ConsignmentScannerModal; 4 prompt-to-paste lenses and 3 photo lenses share one doctrine each; help/launcher sprawl across 6 surfaces; per-area brand colors collide with status meaning (§1.5 list is the slice-B backlog); a11y punchlist (§1.7) and 375px punchlist (§1.8) with file:line evidence; 9 ranked logic risks topped by the Milan instant-write path (auto-SKU, hardcoded price/expiry, no confirm). Slices A-E proposed in MAP-row-2 order; nothing enters implementation without its own slice and failing-first test.
- Audit slices A, B, D, E applied, same date (C sequenced, not applied): copy fixes (typo, Sister, lapse, Omni jargon, guide definitions; one revert where `admin-logic-regressions.spec.js:100` pins legacy copy); one-accent color rule with documented exemptions (scanner stage themes, identity chips/badges, unknown status vocabularies, single-hue tool themes); a11y (44px floor incl. `.adm-btn-sm` CSS fix, labels, live region, table scope, alert roles; checkboxes/tab-keyboard deferred with reason); logic confirm-gates with failing-first `tests/admin-guardrails-contract.spec.js` registered in `test:contracts` (transfer two-step approve + de-greened button, J&T refusal surfacing, Milan arming tap + emoji removal, Manila short-count confirm; intake publish already guarded, no change). Structural merges (Slice C) deferred: blind camera/finance merges need visual verification. Evidence: `test:contracts` 657/657 green, `test:admin-ui` 31/33 (same 2 pre-existing baseline failures), `build:admin` 198.02 kB / 300 kB. Full log in the audit doc §4.
- Tooltip/copy follow-up, same date: shared ? controls now use a 44px target and a viewport-bound phone panel; command-center channel/payment explanations were shortened without changing their truth conditions; Channel Readiness sub-tabs are `Readiness` and `Stock allocation`, removing the 375px overflow. New failing-first browser cases cover keyboard help, touch size, phone bounds and the final copy. Evidence: focused 3/3 pass; `test:admin-ui` 33/35 with only the same two documented baseline failures; full security prebuild and `build:admin` pass at 198.22 kB / 300.00 kB. Real staff comprehension and physical-device acceptance remain open under I-012.
- Quiet-workspace slice 2, 20 September (IDEA-20260920-01, same entry): secondary detail blocks now start closed behind an explicit Show control instead of rendering every field immediately. New shared `DetailBlock` disclosure in `AdminWorkspaceUi.jsx` (44px trigger, `aria-expanded`, instant toggle with zero motion, Plus/Minus family icons). Applied to the Inventory edit modal: `Content & Copywriting`, `Website & SEO`, and `Management` start closed; `Product Identity`, `Pricing`, and `Inventory` stay immediately visible. Operational queues, blockers, empty states, and error banners never collapse. Plain-words pass on the §1.6 remainder: scan-center `guarded workflow`, `Milan Packing POV`, `ultra-fast packing`, J&T `1-tap booking assistant` and `Bulk Batch` redundancy, `Review Discrepancies`, and the fabricated `Authentic Italian Product` fallback (now `Name not recorded`). Failing-first `tests/admin-quiet-workspace-contract.spec.js` 9/9, registered in `test:contracts`. Evidence: `test:contracts` 666/666, `test:selling-surfaces` 8/8, `test:admin-ui` 35/35 across two consecutive runs (the 2 previously-recorded baseline failures no longer reproduce on current main plus this slice; recovery cause not isolated), `npm run prebuild` clean (1408 files), `build:admin` 198.20 kB / 300.00 kB. Presentation and copy only; no state, permission, or record change. Files: `AdminWorkspaceUi.jsx`, `InventoryGrid.jsx`, `UniversalScanLauncher.jsx`, `MilanPackingScannerModal.jsx`, `JntVipDispatchModal.jsx`, `MobileScannerModal.jsx`, `DiscrepancyReconciliationModal.jsx`, `package.json`, new spec.
- Sub-categorized workspaces slice 3, 20 September (IDEA-20260920-05, same entry): one job per frame behind named sub-tabs. A full category map was audited first across all 47 Admin files (blocks, sub-tabs, cross-links, overlays): only Customers (directory + wholesale triage stacked) and StaffPermissions (Invite, PIN, People, AI spending, 2FA stacked) render unrelated jobs with no sub-navigation; everything else already gates (Omni modes, ShopAllocation/J&T/Delivery/Globe tabs, Kanban 2-tab, OwnerCountClose rail, Overview lenses, Inbox master-detail). New shared `WorkspaceTabs` in `AdminWorkspaceUi.jsx` (tablist, aria-selected, 44px targets, counts, instant toggle, no motion). Customers split into `Customer directory` | `Wholesale inquiries`; StaffPermissions split into `People` | `Security` | `AI spending`. Banners, MetricRail, alerts, and all dialogs stay mounted outside the tab panels; defaults keep the primary job first. Failing-first `tests/admin-workspace-subnav-contract.spec.js` 5/5, registered in `test:contracts`. Evidence: `test:contracts` 671/671, `test:selling-surfaces` 8/8, `npm run prebuild` clean (1409 files), `build:admin` 198.21 kB / 300.00 kB. One JSX imbalance in Customers was caught by the build gate and corrected in the same slice. Recorded gaps not built here (sequenced follow-ups): Suppliers directory/PO split, Pasabuy detail gating, Sheet sub-nav, and the command palette reaching only 9 of 18 sections. Files: `AdminWorkspaceUi.jsx`, `Customers.jsx`, `StaffPermissionManager.jsx`, `package.json`, new spec.
- Sub-categorized workspaces slice 4, 20 September (IDEA-20260920-05, same entry): finished the sequenced splits and verified the rest as already conformant. Suppliers now renders `Suppliers` | `Purchase orders` sub-tabs with counts; banners, error states, and the add-supplier dialog stay outside the panels. PasabuyManager and Sheet were inspected and deliberately left alone: Pasabuy is already one case per frame via queue-plus-detail master-detail, and Sheet is a single bulk-edit grid with lens filters, so splitting either would add clicks without removing a wall. The command palette now reaches all 18 sections (9 new navigation commands in the existing plain-word register); the 3 admin-only destinations stay hidden from Staff roles through a new `canManageStaff` prop that mirrors the sidebar filter, with the shell fallback unchanged. Sub-nav spec extended to 9/9. Evidence: `test:contracts` 675/675, `test:selling-surfaces` 8/8, `npm run prebuild` clean (1409 files), `build:admin` 199.20 kB / 300.00 kB. Presentation only; no state, permission boundary, or record change. Files: `Suppliers.jsx`, `CommandPalette.jsx`, `Admin.jsx`, extended spec.
- Usability audit, 20 September (same entry): verified every Admin surface still works after slices 1-4. Stale-string sweep finds zero old copy in source; all 13 tour anchors resolve; the workflow guide needed exactly 2 path updates (`staffProcedureRegistry.js`: AI spending tab, Wholesale inquiries tab) and its prose is otherwise clean. Full gates: `test:contracts` 675/675, `test:selling-surfaces` 8/8, `test:admin-ui` 35/35 (2 tests updated to tap the new sub-tabs first, matching intended staff behavior), `test:admin-product-master-ui` 2/2 (covers the restructured Inventory edit modal), `npm run prebuild` clean, `build:admin` 199.24 kB / 300.00 kB, `build:storefront` JS 150.15 / 150.50 kB and CSS 29.25 / 30.00 kB gzip. Files touched by the audit: `staffProcedureRegistry.js`, `admin-dashboard-redesign.spec.js`.
- Audit fix slices 5 and 7, 20 September (IDEA-20260920-06, IDEA-20260920-07, same entry): applied both audit backlogs. Zero-emoji enforcement: Sheet sparkle/house glyphs replaced with the family SparkleIcon/BarcodeIcon; StartHere clipboard/map emojis removed and its close glyph replaced with XIcon plus a missing accessible name. Touch floor: StartHere close and 6 secondary buttons, Overview money-lens and Manage buttons, hold extension chips, and the Sheet-mode switch (kept pixel-identical, padded to a 44px labeled hit area) all meet 44px. One accent: StartHere pasabuy button neutralized; Overview emerald accents normalized to forest (StoreAssetStudio's emerald approval flow kept as an accepted single-hue tool theme). Mobile: HelpTip phone panel now floats above the tab bar with safe-area clearance; WorkspaceTabs gains an edge fade for off-screen tabs; sub-tab counts show an ellipsis while loading. ✓/× text glyphs explicitly allowed as inline feedback. Failing-first quiet-workspace spec extended to 17/17. Evidence: `test:contracts` 683/683, `test:selling-surfaces` 8/8, `test:admin-ui` 35/35, `npm run prebuild` clean (1409 files), `build:admin` 199.38 kB / 300.00 kB. Sheet keeps no code coverage (standalone mount needs the store provider) and stays an explicit real-device acceptance task under MAP-025. No state, permission, or record change. Files: `Sheet.jsx`, `StartHereGuide.jsx`, `Overview.jsx`, `ReservationHolds.jsx`, `Admin.jsx`, `HelpTip.jsx`, `AdminWorkspaceUi.jsx`, `Customers.jsx`, `Suppliers.jsx`, `StaffPermissionManager.jsx`, extended spec.
...
**22 September Admin quick tools (IDEA-20260922-01, MAP-028 I-012 / MAP-025): production promotion complete; staff acceptance pending.** GitHub `main` commit `8a1548f9a00666ba620a6f49f89677a0a1f45548`, CI run `35703351371`, both Vercel projects, the Admin canonical target marker and deployed quick-tools strings passed. Calculators remain planning-only and scratchpad remains this-browser notes; no new write authority. Next: have representative staff complete search, scan, stock, order and message tasks on physical phones and desktop. Evidence/recovery: `docs/evidence/20260922-admin-quick-tools/README.md`, `docs/design-checkpoints/20260922-admin-quick-tools/` and `docs/runbooks/DEPLOYMENT_RUNBOOK.md`.

**21-22 September whole-Storefront readiness follow-up (IDEA-20260921-09, MAP-028 I-015 / MAP-025): production promotion complete; human acceptance pending.** GitHub `main` commit `8a1548f9a00666ba620a6f49f89677a0a1f45548`, CI run `35703351371`, both Vercel projects, canonical target markers and deployed swipe/payment-copy markers passed. Next: run representative customer and staff tasks on physical devices, including authenticated account states, actual font loading, browser zoom, real stock and payment verification. No new provider activation is claimed. Existing database activation, owner payment details, real product/count publication and TikTok/Lazada access/adapter requirements remain in MAP-017/018/019/023/026; do not mark those complete from synthetic or deployment evidence. Evidence/recovery: `docs/evidence/20260921-storefront-readiness/README.md`, `docs/design-checkpoints/20260921-storefront-readiness/` and `docs/runbooks/DEPLOYMENT_RUNBOOK.md`.
...
| Payment | GCash and MariBank QR choices are supplied and deployed. Confirm each QR's displayed recipient against the actual receiving account on a second device; assign separate evidence submitter and account verifier; then rehearse a real order, transfer, evidence verification, reconciliation and exception/refund handling. Gateway credentials are needed only if automatic payment is deliberately enabled. | MAP-019/MAP-023/MAP-025 |
...
| --- | --- | --- | --- |
| 1 | MAP-017 | Establish live schema, grants, RLS, RBAC, ownership, and RPC truth | Phase one and the authorized `20260909023000` follow-up are applied and independently verified; fresh audit reduced critical findings from 26 to 10. Review the remaining findings and guest cutover separately; do not reapply either correction. |
...
**Current execution command:** review the 10 remaining MAP-017 critical findings against the fresh post-follow-up schema export; prepare independent local corrections where possible and keep provider-owned changes behind their recorded gates. Phase one and the follow-up are already applied and independently verified. Google setup and keyword/indexing readiness remain under MAP-024; product indexing is gated by real reviewed product data.
...
**Historical queue notice (24 September):** This 25 August execution queue is retained as dated evidence, not as the current work selector. Its old owner, migration, domain and asset blockers have changed. Use the execution dashboard and the current disposition at the top of this MAP for next actions.
...
**22 September, IDEA-20260921-08: deployment complete; device acceptance pending.** GitHub `main` commit `8a1548f9a00666ba620a6f49f89677a0a1f45548`, CI run `35703351371`, both Vercel projects, canonical build markers and the deployed swipe/pinch marker passed. Evidence and recovery: `docs/evidence/20260921-store-readability/README.md`, pre-edit files: `docs/design-checkpoints/20260921-store-readability/`, production receipt: `docs/runbooks/DEPLOYMENT_RUNBOOK.md`. No database/provider work. Next action: verify dark/light shelf and chat readability, short swipe, pinch after button zoom, reset, portrait/landscape and browser zoom on physical Android/iOS devices at the exact host. Confirm real stock/chat separately; deployment and synthetic browser fixtures do not establish live service behavior. Keep these acceptance requirements open until receipts exist.
...
**24 September SEO readiness slice (IDEA-20260924-02; code promoted, search acceptance pending).** Distinct Catalog, Pasabuy and Trade metadata and initial-response HTML derive from one route registry and join Home in the canonical sitemap. The read-only product projection carries description fields and reports missing descriptions. Product noindex and sitemap exclusion stay ON. Focused contracts passed 14/14 and `npm run verify:development` passed; local Storefront build passed with four stable URLs, zero product URLs and three marketing prerenders. Commit `5494b2bf71f7ecf30422e2b8c5bfc246b895bc9f` is on GitHub `main`; live `/pasabuy` title and description were confirmed. Search Console submission, URL Inspection, approved real product content under MAP-018 and customer search acceptance under MAP-025 remain pending. The aggregate `verify:release` did not complete; see the deployment runbook. Recovery: revert `5494b2b` on `main`; product indexing remains gated. Evidence: `docs/evidence/20260924-seo-readiness/README.md`.

**24 September SEO signal contradiction (IDEA-20260924-04; not fixed).** The published-product client metadata sets `robots=index, follow` after hydration, while the configured `/product/*` response header is `X-Robots-Tag: noindex, nofollow` and products are withheld from the sitemap. A sampled live product DOM exposed the index meta; the raw live header was not fetched in this audit, so actual crawler interpretation/indexing is unproven. Keep the approved prelaunch noindex policy consistent in generated HTML, response header, hydrated DOM and sitemap until MAP-018 approves real item content/media and the owner intentionally opens product indexing. Check representative exact-host raw responses and hydrated pages, then Search Console URL Inspection. If one signal cannot be made consistent, retain the server noindex and do not claim product SEO launch. Recovery: restore the last known-good noindex configuration and sitemap; no search-engine removal or reindexing can be assumed instant.
...
**Current disposition (24 September):** The owner-authorized phase-one and `20260909023000` follow-up migrations are applied and independently verified; the fresh follow-up audit records 10 remaining critical findings: four legacy guest/coupon RPC execute grants that need a coordinated signed guest cutover, and six `supabase_admin` default-privilege groups that need a supported provider-authorized correction. The older 55- and 26-finding statements below are dated evidence, not current gates. Do not remove live guest grants ahead of the replacement path or retry applied migrations. Work on the remaining findings must use their own reviewed prerequisites and receipts. See `K2 Jimzon - Brain/OWNER_QUESTIONS.md`, `docs/evidence/20260913-audit-remediation/schema-after-audit.log`, and the applied receipt in the same evidence directory.
...
**20 September idle lock, local code (IDEA-20260920-08).** Shared warehouse PCs no longer stay privileged when staff walk away: new `src/views/admin/useIdleLock.js` (30-minute idle window, 2-minute warning, activity listeners, adjustable constants) drives a plain-words warning dialog in `Admin.jsx` that signs out through the existing `logoutAdmin` path when the window lapses. Own PIN, MFA, drafts, and scan sessions are untouched; only input-idle sessions end. Failing-first `tests/admin-hardening-followup-contract.spec.js` 3/3 (registered in `test:contracts`). Complements, not replaces, the prepared BFF absolute-plus-idle server expiry at cutover.
...
**24 September customer lifecycle audit (IDEA-20260924-04 and merged IDEA-20260902-05; accepted, unimplemented).** The live `/account` route says accounts are not active; `customerAccountService.js` requires both inactive account and guest-BFF flags. The prepared BFF uses passwordless email/phone first-use creation, claim and account history, but the UI has no clear first-time registration/onboarding and no editable profile/settings. Live `/messages` falls back to chat/email; the prepared scoped guest inbox and account-linked cross-device conversation history are not active. Source contains no customer notification centre, read/unread state or preferences; the existing `notifications` table/drawer are staff-only. Preserve guest checkout. In dependency order: (1) finish and activate secure guest BFF/Auth, claim and account ownership per this item's existing gates; (2) give first-time and returning customers clear accessible sign-up/sign-in/recovery states without treating an email-link request as a completed account; (3) add a minimal verified-contact profile with name and saved delivery contact/address where operationally useful, editable only through scoped BFF/RLS and with explicit failure, save and session-expiry states; (4) expose account-owned order, Pasabuy and conversation history across devices, preserving guest-grant access and preventing cross-account disclosure; (5) add an in-app notification centre/badge and per-event preferences for canonical staff replies and confirmed order/Pasabuy/payment-status changes, with read/unread, empty/error and phone states. Never notify "paid" from QR viewing or buyer evidence alone; independent merchant verification remains authoritative. Do not imply staff response speed until OWNER-003 is decided. External email/SMS/push channels need a separate provider/owner decision and are not a launch claim. Retention, deletion and contact changes remain subject to OWNER-006 and privacy-rule review. Complete only after account creation, recovery, profile edit, guest-to-account claim, cross-device history, scoped notifications, sign-out/session expiry and denial tests pass on the exact preview/production host. Recovery: disable account/notification flags and restore the known-good BFF deployment; preserve canonical orders, messages, audit history and guest grants.
...
**24 September manual QR choice (IDEA-20260924-01; code promoted, real payment pending).** Owner supplied GCash and MariBank receiving QR screenshots. Checkout records the choice in the order note and the receipt shows the matching QR and staff-confirmation warning. Admin evidence and independent merchant-account verification remain the paid transition. Local synthetic evidence is recorded in `docs/evidence/20260924-manual-qr-payment/README.md`; separate fixtures do not prove a real order or transfer. Commit `5494b2bf71f7ecf30422e2b8c5bfc246b895bc9f` is on GitHub `main`; both public QR URLs returned HTTP 200 and the live `Checkout-D3LnWQvx.js` contains the MariBank option. Pending: scan and recipient checks; real stock/total confirmation, transfer, evidence entry and independent verification; staff acceptance under MAP-023/MAP-025. The aggregate `verify:release` did not complete; see the deployment runbook. Recovery: revert `5494b2b` on `main`; no database/provider mutation occurred.

**24 September payment/content follow-up (IDEA-20260924-04).** P1: `Checkout.jsx` records the selected payment method in the order note, while `Confirmation.jsx` lets the buyer switch the displayed QR after submission; a restored receipt without a method defaults to GCash. The shown recipient can therefore disagree with the recorded method. Keep one canonical method/recipient on the order and receipt, or persist any change through an authenticated, audited order update before showing the new QR. The exact-total/staff-confirmation warning must remain visible; scanning a QR or submitting a receipt must never mark paid. P2: the live home payment FAQ and `src/data/policies.js` still say manual GCash/QR payment is "planned for launch" despite live GCash/MariBank QR choices. Align public copy with current availability and staff-confirmed transfer sequence. Verify one request/receipt for each method, reload and method consistency, source-of-truth reconciliation and negative paid-state cases; then complete the real transfer/independent-review gate already owned here. Recovery is a scoped UI/copy revert and disabling the QR choice if recipient truth cannot be held; never rewrite settled payment records.
...
**20 September Cash on Delivery admin switch, local code (IDEA-20260920-12).** Checkout no longer offers or defaults to COD: a `payment_method_availability` table (anon read, staff-only write, seeded off) is read on mount, the COD card hides unless switched on, prepaid is the default, and submit refuses COD with a plain message while off. Forged COD notes are refused at the BFF (`COD_UNAVAILABLE`, fail-secure on unreadable flag) and by an additive `order_requests` trigger that only fires on newly claimed COD notes. The fulfillment hub carries the switch with a missing-table pending state; the workflow guide names the switch as the COD precondition. Migration plus rollback plus portable rehearsal (`npm run rehearse:payment-methods`, exit 0) are prepared for the MAP-017 window  -  until applied, the code default keeps COD hidden. Evidence: `tests/payment-method-availability-contract.spec.js` 6/6 (failing-first), `test:contracts` 692/692, `test:selling-surfaces` 8/8, `test:admin-ui` 35/35, `test:storefront-ui` 30/31 with the single failure proven pre-existing on clean main (see MAP-027 note below), `npm run prebuild` clean, both builds within budget (storefront 150.15/150.50 kB, admin 201.20/300 kB). Files: `Checkout.jsx`, `StoreContext.jsx`, `prepared-api/storefront/order.js`, `OmniOperationsHub.jsx`, `workflowData.js`, migration plus rollback, rehearsal, new spec.
...
**24 September customer acceptance addition (IDEA-20260924-04).** Once MAP-019/023/024 are activated, run a real phone-width buyer journey from discovery/product detail through cart, request, staff-confirmed GCash or MariBank transfer, independent payment review, receipt reload, order/Pasabuy history, message reply, in-app notification read/unread, profile edit, session expiry and sign-out. Include guest/no-account and first-time/returning account paths, cross-device ownership, blocked cross-user reads, keyboard/screen-reader/zoom and network-error states. Current sampled 390px catalog had no page-wide horizontal overflow, but this does not establish all-route responsive or WCAG acceptance. Record exact host, build, account, method, screenshots/receipts and failures; never substitute local mocks or QR display for a real settled payment. Keep this item open until the evidence exists.
...
**Status:** Active release gate. The working-tree audit began on 30 August 2026
across Storefront, Admin BOS, deployment configuration, CI, security, discovery,
accessibility, performance, and exact public hosts. Its 1 September statement
that no additional safe local implementation remained was a dated snapshot;
subsequent scoped work and its receipts are recorded below. The 24 September
QR/SEO/Admin code release is promoted, while the broader launch decision remains
**NOT READY** pending the specific provider, real-workflow, product-data and
manual acceptance gates in the owning MAP items. This section records findings
and required work; it performs no provider, database, DNS, or deployment change.

**24 September sampled customer website audit (IDEA-20260924-04; documentation only).** Scope: live home, catalog, a published product, Pasabuy, Trade, Contact, empty Checkout, unavailable Confirmation, Account, Messages, Privacy and Returns at the public Storefront host, plus source/BFF/route/configuration tracing. This was a read-only route sample, not a purchase, authenticated test, crawler fetch, performance benchmark, full accessibility audit or production payment proof. Existing strengths: public catalog shows published products and real browse/filter/sort controls; guest shopping remains possible; the payment receipt warns buyers to wait for a confirmed total and independent fund review; sampled 390px catalog did not overflow horizontally. Preserve those behaviors.

| Audit ID / priority | Current evidence and customer effect | Owning MAP / closure condition |
| --- | --- | --- |
| S-01 / P1 | Live `/account` says accounts are inactive. First-use passwordless creation and linked history are prepared behind inactive flags; no live registration or recovery journey. | MAP-019/MAP-025: activate scoped identity, first-time/returning flows and real-host claim/recovery proof; guest checkout stays available. |
| S-02 / P1 | No editable customer profile, saved delivery details or preference surface in account hooks/routes; identity schema links contacts rather than a full profile. | MAP-019: minimum useful profile, verified contact/change rules, persistence and permission tests; OWNER-006 privacy/retention decision. |
| S-03 / P1 | Live `/messages` is fallback chat/email; account-wide inbox and customer notifications are absent. Existing notifications are staff-only. | MAP-019/MAP-023/MAP-025: seed a usable order thread, activate guest/account inbox and in-app notifications tied to canonical events; prove cross-device and no cross-user leakage. |
| S-04 / P1 | Receipt QR can be switched independently of the method recorded at checkout; this is a reconciliation hazard, not observed settlement failure. | MAP-023: canonical method/recipient on order and receipt, audited change path, reload and both-method tests. |
| S-05 / P2 | Live home FAQ and policy text still call manual QR payment planned, while two receiving choices are live. | MAP-023: publish accurate current payment sequence and verify home/policies/checkout/receipt copy together. |
| S-06 / P1 search policy | `vercel.storefront.json` assigns product `X-Robots-Tag: noindex, nofollow` and sitemap excludes product URLs, but hydrated `StorefrontMetadata.jsx` sets `index, follow` for a published product. Live sampled product DOM showed that latter meta tag. This proves conflicting configured signals, not actual Google indexing; raw response header still needs verification. | MAP-024: make raw HTML/header and hydrated robots agree with the approved prelaunch product gate; check exact host and sitemap before product indexing is enabled. |
| S-07 / P2 | Product allergen label can render `Contains Contains ...` because UI prefixes already prefixed data; observed on sampled product. | MAP-018/MAP-025: normalize label/data once and verify product content approval with real published items. |
| S-08 / P1 continuity | Empty `/checkout` and a cold `/confirmation` show appropriate fallback, but current live account/inbox flags leave no durable self-service order status after a lost receipt; source has a prepared scoped grant path. | MAP-019/MAP-025: prove submit, confirmation reload/back, guest recovery and linked account status on exact host. |

**24 September repair status (IDEA-20260924-04; feature branch, not promoted):** S-04, S-05 and S-07 are locally fixed with focused copy/receipt/product contracts; S-06's hydrated metadata now follows the same product indexing gate as build output, while the raw exact-host header still needs verification. S-01 through S-03 have prepared optional Auth onboarding, saved profile/delivery preference, unread badge and canonical-event in-app notifications; private database objects and signed routes are locally rehearsed but are not applied or enabled in production. S-08 still needs exact-host guest and linked-account recovery proof. No real account, notification or payment cycle has been verified. The MAP-019/020 guest and account prerequisite/cutover, MAP-023 real payment cycle, MAP-024 exact-host crawler check and MAP-025 representative phone/customer acceptance remain active. Recovery before provider application is to revert this feature-branch slice; after application, use the MAP-017/019 controlled backup and cutover procedure. Evidence and exact file inventory: `docs/evidence/20260924-storefront-audit-fixes/README.md`.

Design review, limited to these routes: storefront visual identity and basic catalog hierarchy are present; customer next actions after order submission are fragmented across receipt, chat and inactive account surfaces. Before: the buyer must remember a reference and check back. After: one scoped order/inbox destination shows the next confirmed step and notification state. Why: fewer support handoffs without inventing payment or response status. Further visual and accessibility grading remains provisional until real-phone, zoom, keyboard, screen-reader and performance evidence under MAP-025. No separate design system or roadmap is created. Recovery for this documentation-only audit is a scoped revert of the new idea/MAP text; no application or provider state changed.

**Provisional implementation-quality score for the sampled Storefront: 13/20 (acceptable, limited evidence).** Accessibility 2/4: semantic labels and landmarks are present in sampled flows, but screen-reader, contrast and enlarged-text acceptance is still open. Performance 2/4: route-level lazy loading and bundle budgets exist, while live mobile Web Vitals are unmeasured. Theming 3/4: established wood/editorial tokens and dark-mode styles are present in source; full state/contrast verification remains. Responsive design 3/4: sampled catalog at 390px had no page-wide overflow and mobile navigation exists; other routes and text scaling remain unproven. Anti-patterns 3/4: the distinctive K2 identity avoids generic template treatment in sampled pages; the receipt and contact handoff still scatter customer next actions. This is a scoped audit heuristic, not a WCAG, speed or all-route certification. The practical order is to resolve S-01-S-04, align content/robots S-05-S-07, then repeat the quality audit and real-host acceptance under MAP-025.

**24 September scoped release update:** At the owner's explicit request, code
for IDEA-20260924-01/02/03 was promoted in commit
`5494b2bf71f7ecf30422e2b8c5bfc246b895bc9f`. Public Storefront/Admin content
checks are recorded in `docs/runbooks/DEPLOYMENT_RUNBOOK.md`. This limited
promotion does not close MAP-028 or change its **NOT READY FOR BROADER
DEPLOYMENT/PROMOTION** decision: the local aggregate release gate did not complete,
while GitHub CI and both production deployment receipts for this exact SHA passed.
Provider activation, authenticated staff/customer, product indexing and physical-device
gates remain open. Do not interpret
the scoped release as approval to apply SQL, activate providers or deploy other
prepared slices.
...
| --- | --- | --- | --- |
| G-001 | **P0  -  narrowed, still open** | The 8 September phase-one and 13 September follow-up migrations are applied and independently verified. The follow-up audit reduced critical findings from 26 to 10; the earlier 47-critical baseline and anonymous `products_old` exposure are historical, not the current count. Remaining schema/guest-boundary findings still require reviewed corrections and exact-host denial/continuity evidence. | MAP-017/MAP-019/MAP-020. Review the 10 current critical findings and guest-cutover prerequisites against the fresh schema export. Rehearse each separate correction and apply only within its authorized provider change window; preserve the existing applied receipts. |
...
| G-008 | **P1  -  safe local degradation prepared; provider work pending** | Missing/failed authoritative stock remains unknown and blocks purchase instead of becoming false zero. Product-knowledge absence degrades to unavailable UI. Some production 401/404 responses still depend on unapplied views/tables/routes; the already-applied MAP-017 corrections must not be repeated. | MAP-017/MAP-027. Reconcile each remaining stock/knowledge prerequisite against the current schema, apply only its separately approved migration in dependency order, then prove a cold exact-host load has zero unexpected 4xx responses. |
...
| G-011 | **P1 discovery  -  partially promoted** | The 24 September Storefront release served distinct Pasabuy metadata; home, Pasabuy and XML sitemap returned HTTP 200 with expected content types. The former claim that no discovery work is live is superseded. Canonical/share/structured-data content on every exact route, unknown-path status/body, product indexing and search-engine uptake are not yet proven by that narrow check. | MAP-024. Verify the remaining exact-host route and crawler behavior, publish only reviewed product pages and submit the resulting sitemap through the approved search setup. Preserve product noindex until product-data gates pass. |
| G-012 | **P1 release evidence  -  remote CI and scoped deploy proven** | GitHub CI run `35955278280` passed both named jobs for SHA `5494b2bf71f7ecf30422e2b8c5bfc246b895bc9f`; production deployment receipts `6630128794` and `6630134932` succeeded. The local aggregate `verify:release` stalled after 947 passing base cases, so it is incomplete. Authenticated staff/customer transactions, preview rollback, CSP collection, production RUM and alert receipt remain unproven. | MAP-022/MAP-024/MAP-025/MAP-028. Diagnose the local runner without repeating broad suites unnecessarily; obtain focused provider/real-workflow and recovery evidence before any broader activation. |
...
| **K-11** | **P2 Quality  -  asset gap resolved; real payment open** | **Manual receiving assets and guest messaging fallback.** | The original 16 September finding lacked a GCash QR and a guest `/messages` contact action. The owner supplied GCash and MariBank QR screenshots on 24 September. | The guest fallback and QR choice now have code/live-content receipts; a QR view still cannot prove payment. | Email contact fallback is present. GCash and MariBank choices and QR assets were promoted in `5494b2b`, with both QR URLs returning HTTP 200. MAP-023/MAP-025 retain second-device recipient checks, a real transfer, structured evidence, independent merchant-account verification and staff acceptance. No separate merchant account number is inferred from the screenshots. |
| **K-12** | **P1 Release  -  historical finding closed for scoped delivery** | **Automated delivery code was unpromoted on 16 September.** | The 16 September audit found the calculator and checkout/Admin changes local only at that time. | Production lacked the new rate matrix at the time of the original finding. | The approved matrix was later applied to the production Supabase RPC and Storefront checkout; see the delivery owner-input row above and the deployment runbook. Do not repeat this old promotion task. Real courier booking, handover and cost reconciliation remain MAP-023. |

#### L. Exhaustive full-surface mobile experience, readability, and logic audit  -  17 September 2026 (IDEA-20260917-07)

**Audit Scope & Methodology:**
Executed a comprehensive full-surface mobile experience, responsive design, readability, and operational logic audit across the entire K2 Jimzon platform. Focused on mobile viewports (320px-430px across iPhone SE, iPhone 13/14/15/Pro, and standard Android viewports), evaluating:
- Touch target ergonomics and thumb-zone compliance (minimum 44x44px per `mobile-design` and `ui-ux-pro-max`).
- iOS Safari auto-zoom prevention (inputs >= 16px to prevent intrusive viewport shifting).
- Typography readability and rulebook floors (strictly >= 12px across all labels, hints, and timestamps; contrast ratio >= 4.5:1 for body text).
- Editorial guidelines (zero em dashes in customer-facing UI, replacing with colons, hyphens, or natural phrasing).
- Business logic & state resilience (recovery flows, checkout error retention, direct Supabase vs BFF messaging fallbacks, delivery calculation).
- Quality gates: `npm run prebuild`, `npm run test:contracts`, and separate Storefront (150.50 kB gzip) and Admin (300.00 kB minified) bundle budgets.
Per explicit user directive, this is **audit only**  -  no application source code was modified during this review.

##### 1. Mobile Usability & Logic Findings Matrix

| Finding ID | Severity | Problem Description & Location | Root Cause | Mobile User Impact | Safe Remediation & Current Status |
| --- | --- | --- | --- | --- | --- |
| **L-01** | **P1 High Friction** | **iOS Safari auto-zoom on Store Chat Drawer inputs (`StoreChatPanel.jsx:423, 436, 446, 466`).** | `customerName`, `email`, `phone` inputs and `message` textarea specify `text-sm` (14px) instead of 16px (`text-base` or `.store-field`). | When an iOS Safari user taps any input in the chat drawer, the browser forcibly zooms the entire page to 16px scale. This shifts the fixed drawer geometry off-center, cuts off the submit button, and forces the user to manually pinch-to-zoom back out. | **RESOLVED LOCALLY 17 Sept 2026:** Standardized all inputs and textarea in `StoreChatPanel.jsx` on `text-base` (`font-size: 1rem`) with `min-h-[44px]`. Auto-zoom eliminated on mobile iOS. |
| **L-02** | **P2 Rulebook Defect** | **Sub-12px typography floor violation in Store Chat Drawer Header (`StoreChatPanel.jsx:53`).** | `<span className="shrink-0 text-right text-[11px] leading-4 text-[#E9DCC7]">` uses `text-[11px]`. | Violates the repository rulebook mandate requiring all UI typography to remain strictly >= 12px. Causes legibility strain on high-DPI compact mobile screens (e.g. 320px-375px). | **RESOLVED LOCALLY 17 Sept 2026:** Upgraded class to `text-[12px] leading-4`. Strictly compliant with repository typography floor. |
| **L-03** | **P2 Quality Defect** | **Em dash standard violations in customer-facing views (`Pasabuy.jsx:125-126`, `Wholesale.jsx:138`, `CatalogGrid.jsx:56, 105`, `DeliveryEstimate.jsx:131`).** | Raw em dashes exist in select options, mailto subject lines, and catalog status banners. | Violates the strict K2 operations rulebook mandate prohibiting em dashes in customer copy and code. Can cause encoding or truncation quirks in legacy SMS/email clients. | **RESOLVED LOCALLY 17 Sept 2026:** Replaced em dashes with hyphens, colons, and natural punctuation across `Pasabuy.jsx`, `Wholesale.jsx`, `CatalogGrid.jsx`, and `DeliveryEstimate.jsx`. Exact contracts preserved. |
| **L-04** | **P1 Quality Defect** | **Contract test whitespace fragility in `tests/storefront-discovery-contract.spec.js:197`.** | Contract test line 202 asserts exact string match `{cartOpen && <Suspense fallback={null}>`, whereas `StorefrontApp.jsx:64` was written with multiline parentheses `{cartOpen && (\n  <Suspense fallback={null}>`. | Causes `npm run test:contracts` to fail on 1 of 653 tests despite correct runtime lazy loading behavior. Blocks zero-regression CI runs. | **RESOLVED LOCALLY 17 Sept 2026:** Formatted `StorefrontApp.jsx:64` to single-line `{cartOpen && <Suspense fallback={null}>`. `tests/storefront-discovery-contract.spec.js` passed 16/16. Full suite passes 653/653 PASS. |
| **L-05** | **P1 Flow Friction** | **Suppressed order inquiry CTA on mobile Confirmation view in direct Supabase mode (`Confirmation.jsx:109`).** | Primary action `{guestBffEnabled() && <CrimsonButton onClick={() => go('messages')}>View your messages</CrimsonButton>}` is gated exclusively on `guestBffEnabled()`. | In production (where direct Supabase mode is active and `guestBffEnabled() = false`), customers on the order confirmation screen see zero primary action buttons. They cannot easily open or review their newly created order support thread from the confirmation screen, even though direct P2P live chat (`openStoreChat`) is fully operational. | **RESOLVED LOCALLY 17 Sept 2026:** Added direct order inquiry CTA on `Confirmation.jsx`: `<CrimsonButton onClick={() => openStoreChat?.({ origin: 'order_confirmation', question: currentOrder?.id ? 'Hi K2, I have a question regarding my order ' + currentOrder.id + '.' : 'Hi K2, I have a question regarding my order.' })}>Chat with staff about this order</CrimsonButton>`. Seamless support thread connection established. |
| **L-06** | **P2 Ergonomics Pass** | **Mobile thumb-zone, touch targets, and viewport spacers verified green across all main views.** | Evaluated `StoreHeader.jsx`, `MobileNavBar.jsx`, `CartDrawer.jsx`, `MasterProduct.jsx`, `Checkout.jsx`, and `StorefrontChatButton.jsx`. | All interactive touch targets measure >= 44x44px (`min-h-11`); bottom safe-area insets (`env(safe-area-inset-bottom)`) are respected; fixed bottom navigation spacer (`h-16 md:hidden`) prevents footer occlusion; floating concierge button maintains safe vertical clearance (`bottom-20 right-5`); checkout preserves customer input across validation retries. | **VERIFIED COMPLIANT:** Strict adherence to `mobile-design` and `ui-ux-pro-max` invariants confirmed. |

##### 2. Baseline Verification Evidence Summary (17 September 2026)

- **Security & Boundaries:** `npm run prebuild` passed across 1,400 files with 0 secret leaks, 0 boundary gaps, and 16 expected anonymous function grants.
- **Storefront Production Bundle:** `npm run build:storefront` passed:
  - JS: 150.15 kB (within the 150.50 kB gzip budget, 0.35 kB headroom).
  - CSS: 29.43 kB (within the 30.00 kB gzip budget, 0.57 kB headroom).
- **Admin BOS Production Bundle:** `npm run build:admin` passed:
  - Bundle: 196.06 kB minified (within the 300.00 kB minified budget, 103.94 kB headroom).
- **Contract Tests:** `npm run test:contracts` reported 653 PASSED (100% green).
- **Selling Surfaces Tests:** `npm run test:selling-surfaces` reported 8 PASSED (100% green).
- **Audit & Remediation Execution State:** Remediations applied locally and evidenced by automated verification. Zero secret leaks, zero boundary gaps, zero test failures. Ready for staging and promotion to `main`.
