...
| --- | --- | --- | --- |
| **Immediate** | `OWNER-005` remaining database permission gates | MAP-017 reconciliation of 13 current critical findings | The owner reopened the scoped stock correction with “bro fix it.” The automatic approval review blocked uploading the new encrypted backup to Drive. The only owner action now is to authorize that exact backup destination; Codex handles the technical upload and scoped database change after the recovery gate passes. |
