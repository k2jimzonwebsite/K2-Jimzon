...
**Current 25 September outcome:** The owner reopened the earlier deferral with “bro fix it” and explicitly authorized uploading the encrypted backup plus both redacted companion files to the named owner-only K2 Production Backups folder. Codex uploaded and independently read back all three files, applied only the reviewed stock `PUBLIC` grant revocation as production migration `20260925111537`, and verified public stock and catalog continuity. The stock owner decision is fulfilled. MAP-017 retains six direct guest grants and six `supabase_admin` default-privilege findings; the latter await Supabase support. Earlier paragraphs below document the decision and approval-review sequence and are superseded for current status. Evidence: `docs/evidence/20260925-map017-stock-apply/README.md`.

**Stock grant decision, 25 September: Deferred by the owner.** The owner replied “Defer the stock correction.” The original phase-one and `20260909023000` approvals below are complete and do not authorize this new payload. The prepared, unapplied `supabase/migrations/20260925_map017_stock_public_grant_revocation.sql` has SHA-256 `479BCCE8AFE8EEA68B7C89F85370F5EE06FC28268ADC7DF9BC155D236E88A94D`; its emergency-only rollback has SHA-256 `61A929C103298F43DB96E819EF1C6E3E8DCBE7DEF61EAEC8161CBB26B6B5C470`. The 25 September read-only live check confirmed the extra grant remains, local rehearsal passed, and an encrypted application-database backup passed an isolated restore. **Do not apply this production correction or treat the backup as approval. Do not ask the owner to repeat the decision.** If the owner later reopens it, Codex must refresh live checks and recovery evidence, then present the exact change for a new decision. Independent guest preparation and Supabase provider follow-up may continue. No production SQL changed.

**Later 25 September owner reopening and current owner-only action:** The owner replied “bro fix it” after the deferral, reopening and authorizing the previously reviewed scoped stock correction, SHA-256 `479BCCE8AFE8EEA68B7C89F85370F5EE06FC28268ADC7DF9BC155D236E88A94D`. Codex confirmed live `PUBLIC=true`, explicit `anon=true`, explicit `authenticated=true`, 21 stock rows, public RPC HTTP 200/21 rows and exact-host catalog HTTP 200. A new encrypted database backup `current-pixplcjqivlfflickobf-2026-09-25T100842718Z-dde4432c4355` passed an isolated restore and exact migration/rollback practice on the restored application schema. The automatic approval review **rejected** uploading this sensitive backup to the existing unshared `K2 Production Backups` Google Drive folder under `k2jimzonwebsite@gmail.com` (folder `1mQuU8Jj6eWhDr-lpZV3YJDtaEwfAh8yo`): it said the destination was not established by trusted user authorization. No workaround or production apply occurred. **Only owner action:** explicitly authorize or decline uploading this encrypted backup to that named owner-only folder. Codex will perform the upload, verify its metadata/retrieval, then apply and verify only the scoped stock grant change if the recovery gate is satisfied. The owner need not handle files, SQL, or dashboard controls.

**Later 25 September owner approval and remaining companion-file decision:** The owner explicitly authorized uploading the encrypted backup to the named K2 folder. Codex uploaded it as Drive file `1iPIOt0Z-sd8g4a3g4zhJj5VGgwKHOb3M`; metadata confirmed correct parent, 825,060 bytes, unshared, owned by `k2jimzonwebsite@gmail.com`. Independent download SHA-256 matched the local encrypted envelope. Automatic approval review rejected the next upload because the separate **redacted manifest** was outside that explicit authorization; it contains project/backup identifiers, timestamps and checksums, but no credentials or customer rows. The manifest is required to restore the envelope after local loss. **Only owner action now:** explicitly approve or decline uploading the redacted manifest and restore-verification receipt for this same backup to the same owner-only folder. Do not bypass the rejection. No production grant has changed.
