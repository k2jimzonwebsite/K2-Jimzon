...
import { intakeAiReadiness } from './intake-ai-provider.js'
...
export default async function handleSystemReadiness(req, res) {
  ...
  if (!isAdminRole(authorized.identity.role)) return safeJson(res, 403, { error: { code: 'SYSTEM_READINESS_ADMIN_REQUIRED' } })
  ...
  return safeJson(res, 200, { ok: true, readiness: { ...data, serverBoundary: true, currentSessionAal2: true, automaticIntakeConfiguration: intakeAiReadiness() } })
}
