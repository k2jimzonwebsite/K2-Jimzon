import {
  idempotencyKey, publicFailure, readJson, requireAllowedOrigin,
  requireStorefrontProject, safeJson, signedRpcArguments, text,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data, error } = await client.rpc(
      'append_guest_message_v1', signedRpcArguments(req, 'guest_reply', payload)
    )
    ...
    return safeJson(res, 201, {
      ok: true,
      receipt: { message_status: mapped.result.message_status, created_at: mapped.result.created_at },
    })
  } catch (error) {
