...
test.describe('admin command center redesign', () => {
  ...
  test('quick tools expose useful shortcuts, safe expiry guidance and keyboard recovery', async ({ page }) => {
    test.setTimeout(240000)
    await page.route(/^https:\/\/fonts\.(googleapis|gstatic)\.com\//, route => route.abort())
    await page.addInitScript(() => localStorage.setItem('k2_tools_pos', JSON.stringify({ x: 5000, y: 5000 })))
    await page.setViewportSize({ width: 390, height: 844 })
    await page.goto('/admin-portal-k2-secure', { waitUntil: 'domcontentloaded' })
    await expect(page.getByRole('heading', { name: 'Operations command center' })).toBeVisible({ timeout: 45000 })
    const trigger = page.getByRole('button', { name: 'Open Admin tools' })
    await expect(trigger).toBeInViewport()
    await trigger.click()
    const tools = page.getByRole('dialog', { name: 'Staff Tools and Margin Planner' })
    await tools.getByRole('button', { name: 'Quick actions', exact: true }).click()
    for (const name of ['Find a product or page', 'Scan a barcode', 'Products and stock', 'Orders and packing', 'Customer messages', 'Workflow map', 'Keyboard shortcuts']) {
      await expect(tools.getByRole('button', { name, exact: true })).toBeVisible()
    }
    await tools.getByRole('button', { name: 'Expiry', exact: true }).click()
    await tools.getByLabel('Expiry date').fill(daysAhead(60))
    await expect(tools.getByText(/Below the 90-day arrival rule/)).toBeVisible()
    await tools.getByLabel('Expiry date').fill(daysAhead(120))
    await expect(tools.getByText(/date threshold only/)).toBeVisible()
    await tools.getByRole('button', { name: 'Scratchpad', exact: true }).click()
    await tools.getByLabel('Private browser scratchpad').fill('Test note only')
    await page.keyboard.press('Escape')
    await expect(tools).toBeHidden()
    await expect(trigger).toBeFocused()
    await trigger.click()
    await expect(tools.getByLabel('Private browser scratchpad')).toHaveValue('Test note only')
    await tools.getByRole('button', { name: 'Quick actions', exact: true }).click()
    await expect(tools.getByRole('button', { name: 'Quick actions', exact: true })).toHaveAttribute('aria-pressed', 'true')
    await page.screenshot({ path: 'docs/evidence/20260922-admin-quick-tools/mobile.png', animations: 'disabled' })
    await tools.getByRole('button', { name: 'Keyboard shortcuts', exact: true }).click()
    await expect(tools).toBeHidden()
    await expect(page.getByRole('dialog', { name: 'Keyboard shortcuts' })).toBeVisible()
    await page.keyboard.press('Escape')
    await page.setViewportSize({ width: 1440, height: 900 })
    await trigger.click()
    await page.screenshot({ path: 'docs/evidence/20260922-admin-quick-tools/desktop.png', animations: 'disabled' })
    await tools.getByRole('button', { name: 'Workflow map', exact: true }).click()
    await expect(tools).toBeHidden()
