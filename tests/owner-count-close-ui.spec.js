...
test('phone Owner Count & Close reaches a sealed customer-free handoff', async ({ page }) => {
  ...
  await expect(page.getByText('3. Review product matches')).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: '4. Check sales and duplicate orders' })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: '5. Estimate marketplace fees' })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: '6. Compare recorded and counted stock' })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: '8. Check stock by shop' })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: '9. Check Pasabuy packing readiness' })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: '10. Prepare the bookkeeping handoff' })).toBeVisible()
