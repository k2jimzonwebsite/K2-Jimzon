...
for (const width of [390, 1440]) for (const dark of [false, true]) {
  test(`readable shelf and chat at ${width}px in ${dark ? 'dark' : 'light'} mode`, async ({ page }) => {
    ...
    const railButton = store.locator('.k2-store-rail button').first()
    await expect(railButton).toBeVisible()
    expect(await contrast(railButton)).toBeGreaterThanOrEqual(4.5)
    await railButton.click()
    const productPopup = store.locator('.k2-store-product-popup')
    await expect(productPopup).toBeVisible()
    await expect(productPopup.locator('h2').first()).toBeVisible()
    expect(await contrast(productPopup.locator('h2').first(), '.k2-store-product-popup')).toBeGreaterThanOrEqual(4.5)
...
test('short mobile drag changes shelf; zoom controls and hint leave room to browse', async ({ page }) => {
  ...
  await expect.poll(cameraDistance).toBeLessThan(originalDistance * 0.98)
  await expect(page.locator('.k2-store-zoom')).toBeHidden()
  const navTabs = page.getByRole('navigation', { name: 'Shelves', exact: true })
  for (const name of ['Coffee & Drinks', 'Pantry & Oil']) {
    const button = navTabs.getByRole('button', { name, exact: true })
